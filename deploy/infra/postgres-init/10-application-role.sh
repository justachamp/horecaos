...
#
# Why two roles at all. `POSTGRES_USER` is `horecaos_migrator`: it owns every
# object, and it is the only role that can change the schema. The application
# connects as `horecaos_app`, which owns nothing and holds only the privileges each
...
# application as `horecaos` — superuser, and owner of the database — so every GRANT
...
# compose.yaml passes HORECAOS_APP_PASSWORD directly — the same worthless-outside-a-
...
elif [ -n "${HORECAOS_APP_PASSWORD:-}" ]; then
    app_password="${HORECAOS_APP_PASSWORD}"
else
    echo "!! Neither ${app_password_file} nor HORECAOS_APP_PASSWORD is set." >&2
...
# statement safely.
export HORECAOS_APP_PASSWORD="${app_password}"
unset app_password

psql --username "${POSTGRES_USER}" --dbname "${POSTGRES_DB}" \
     --set ON_ERROR_STOP=1 --no-psqlrc --quiet <<-'SQL'
	\getenv app_password HORECAOS_APP_PASSWORD
	...
	DO $$
	BEGIN
	    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'horecaos_app') THEN
	        CREATE ROLE horecaos_app LOGIN;
	    END IF;
	END
	$$;
	...
	ALTER ROLE horecaos_app SET statement_timeout = '60s';
	ALTER ROLE horecaos_app SET idle_in_transaction_session_timeout = '60s';
	ALTER ROLE horecaos_app SET lock_timeout = '5s';
SQL

unset HORECAOS_APP_PASSWORD

echo "==> horecaos_app created; the application will connect as a non-owner"
