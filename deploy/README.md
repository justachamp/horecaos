...
| `unattended-boot.sh`, `systemd/` | Opt-in, per host, and only with a TPM 2.0 (ADR 0080): `enrol` seals three unseal shares and a narrow OpenBao credential to this host's TPM and installs `horecaos-boot.service`, which runs `session-start.sh --unattended` at every boot, so the stack comes back with nobody present. `status` shows the last run without decrypting anything; `revoke` deletes the units, the stored credentials and the `horecaos-boot` role. A host that does not enrol is unsealed by hand, as before. |
...
| `infra/openbao/` | OpenBao server config, agent config (the one path the application reads a secret *value* rather than a *reference* for), and the least-privilege policies — `horecaos-boot` is the one a host enrolled for unattended restart holds (ADR 0080). |
