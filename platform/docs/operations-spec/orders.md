...
| `OrderActionCode`'s widened set past `AMEND` — `COMPLETE`, `RESOLVE`, `ASSIGN_COURIER`, `ISSUE_INVOICE` (ADR 0105) | `COMPLETE` → the wave that owns the completion dialog (`P09`); `RESOLVE`'s capability is itself an ADR 0105 open input; `ASSIGN_COURIER` → `P11`; `ISSUE_INVOICE` → `P12` | The four codes are declared in `OrderActionCode` — naming the endpoint each targets — but `OrderActionsPolicy.availableFor` never emits them: each needs state the (status, mode, `grantedCapabilities`) signature does not carry (a fulfilment-mode-aware completion reason, one amendment's own confirmation state, a courier-assignment path that does not reach an order yet, an invoice-issue endpoint that does not exist yet). `APPROVE`/`REJECT`/`ADVANCE`/`CANCEL`/`AMEND` are principal-aware and **built** — §4.2's own rule ("the client never computes availability from a status string") now holds by construction: every action offered is gated on the exact capability its endpoint declares |
