...
- Implementation status: Built — V0202's `commercial.statements` and `commercial.statement_lines` with their freeze triggers, `StatementService`, `CommercialStatementController` with CSV export, tested against the migrated schema in `ModulesStatementsAndArrearsTests` and `CommercialStatementCsvTests`; the control-plane statements screen and, as of Finance 8/X.4, the tenant-facing one — `CommercialOperationsController.statements`/`.oneStatement`/`.statementExport` mirror the same `StatementService` read at a path the operations console's OpenAPI group can reach, and `frontend/operations`'s `q-subscription-page` renders it. How a statement is paid is now ADR 0095's and built: issuing pays it from the tenant's wallet at once, voiding gives back what it drew, and the screen shows each statement's paid and due amounts, all derived from the ledger rather than stored on the frozen row. Sending it to Didox or Faktura.uz is ADR 0096's
