...
- [x] Implement the ledger, period stamping at insert, prior-period adjustments,
      ...
      ADR 0045's.
- [x] Implement manual adjustments with ADR 0027 approval and an
      outcome-based reason-code registry, whose `outcome_basis` is a closed
      column so a behavioural reason cannot be authored. **This item used to
      read "rule and manual adjustments" and be checked; T16 (ADR 0108) splits
      it, because manual adjustments were genuinely complete while rule
      adjustments were not, and a single checkbox could not say both — see
      the two items below for what T16 actually built.**
- [x] T16 (ADR 0108): give the registry an amount and a typed condition —
      `courier_adjustment_reasons.rule_amount_minor`/`rule_comparator`/
      `rule_threshold`/`rule_window`/`rule_trigger` (V0260), all six or none
      per `ck_adjustment_reason_rule_pair` — and an evaluator,
      `AdjustmentRuleEvaluator`, covering `DELIVERED_VOLUME`, `LATE_DELIVERY`,
      `ON_TIME_RATE` and `GEO_UNVERIFIED_RATE`. It posts
      `AdjustmentOrigin.RULE` from its own Java call only, never from a
      request — `OperationsCourierController`'s `AdjustmentRequest` no longer
      has an `origin` field at all, closing the gap the operations gap map
      named: a client that set `origin=RULE` used to skip the `MANUAL`-only
      branch of the four-eyes check. `ORDER_UNDELIVERED` and `ORDER_DAMAGED`
      have no evaluator reader (they name facts in
      `fulfillment.delivery_exceptions`, which nothing here queries) and may
      only be authored manual-only.
- [ ] **Not built: `SETTLEMENT_PERIOD`-window rule evaluation is not called
      from production.** `AdjustmentRuleEvaluator.evaluatePeriodClose` exists
      and is tested directly; `SHIFT`-window evaluation is wired into
      `CourierShiftService.close`/`approveHours`, but wiring
      `evaluatePeriodClose` into `CourierSettlementService.close` needs it to
      run *before* that method reads `entriesOf`/`earningsOf`/`computeTotals`
      and hashes the result — otherwise a rule-posted entry lands outside the
      very statement it was meant to affect. T16 chose not to reorder that
      method's statement-hashing sequence to fit this in.
