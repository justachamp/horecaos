...
- [x] Close a stream on token expiry and on `TenantGrantsChanged`; rate limit
      ...
      back.
- [x] Enrol the kitchen display device principal through a pairing-code
      handshake adapted from the OAuth 2.0 device authorization grant — see
      ADR 0079, decided and built 2026-09-09. The device principal is a real
      ADR 0025 model change: a non-human principal type with its own
      enrolment and revocation, provisioned as its own confidential Keycloak
      service-account client rather than signed in as a person, which is the
      shared-login problem this bullet used to name as unsolved.
- [ ] **Not built.** Stop a device's SSE stream outside the location's
      service hours. This bullet is now a stream-transport gap rather than an
      identity one: a device authenticates as itself, per ADR 0079, but
      nothing in `telemetry` yet opens or closes a stream on a service-hours
      schedule for any principal, human or device — see ADR 0036 for the
      hours themselves.
...
- ADR 0004 transactional outbox; ADR 0014 delivery sourcing and partner
  ...
  reporting and the metric layer; ADR 0079 kitchen display device principal and
  enrolment, the ADR 0025 model change this record named and did not build.
