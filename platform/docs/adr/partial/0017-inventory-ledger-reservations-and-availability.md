...
- [ ] Implement durable expiry leases and restart recovery. The sweep half is done: `InventoryReservationSweeper` runs `InventoryService.expireStaleReservations` on a `@Scheduled` pass, so a lapsed hold is now released on a clock rather than only when its own quote is re-reserved. **The lease half is not**: the sweep is frequent, best-effort and log-and-continue, with no durable lease and therefore no restart recovery — two instances sweep the same rows and a crash mid-sweep is recovered only by the next pass.
...
- [ ] Add concurrency, failure/replay, and production-shaped load tests. `InventoryReservationAndAvailabilityTests` (470 lines, 7 `@Test` methods) now covers the hold lifecycle — commit, release, TTL-based sweep with a mutable clock — and cross-tenant isolation, closing the "no test class at all" gap this line used to name; a race/concurrency suite and load tests remain unwritten.
