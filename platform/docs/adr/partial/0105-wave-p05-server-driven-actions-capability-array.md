...
| Code | Gated on | Endpoint |
|---|---|---|
| `APPROVE` / `REJECT` | `ORDER_APPROVE` | `POST .../approval-decisions` |
| `ADVANCE` | `ORDER_ADVANCE` | `POST .../state-actions` |
| `CANCEL` | `ORDER_CANCEL` | `POST .../cancellations` |
| `AMEND` | `ORDER_AMEND` | `POST .../amendments` |
...
`OrderActionCode` widens from four values to nine. `AMEND` is emitted today,
gated as above, whenever the order has not ended — mirroring
`OrderAmendmentService.propose`'s own guard, which refuses only a terminal
order; the seven financial amendment commands' own "cut point" (ADR 0030) is a
fact this status/mode pair cannot see and stays enforced only at the endpoint,
exactly as §4.2 already expects for a temporarily-blocked case. `COMPLETE`,
`RESOLVE`, `ASSIGN_COURIER` and `ISSUE_INVOICE` are declared — named in the
enum, in `orders.md` §11, and in the frontend's `ORDER_ACTION_CODES` — but
**not yet emitted**: each targets state `availableFor`'s three-argument
signature cannot carry (a fulfilment-mode-aware completion reason from the
tenant registry; one specific amendment's own confirmation state; a courier
assignment path that does not reach an order yet; an invoice-issue endpoint
that does not exist yet). Declaring them now, inert, means the wire contract,
`OperationsOrderController`'s own class doc and the frontend's
forward-compatible rendering (an unrecognised code renders its raw name,
never nothing) all agree on the full action vocabulary before every one of
them has a route — the same reason the frontend already renders an unlisted
code harmlessly rather than crashing.
...
- The wire contract names the full action vocabulary orders.md §4 already
  documents, ahead of the waves that wire each remaining code — `P09`, `P11`
  and `P12` inherit a named target rather than inventing one.
...
- Four of nine `OrderActionCode` values are declared and inert. A reader of
  the enum who does not also read its class doc could reasonably expect
  `COMPLETE` to appear in a live response today; it does not, until `P09`
  wires it.
...
Declaring four inert codes ahead of their endpoints trades a moment of
...
`P11` and `P12` each land. The alternative — adding each code only when its
endpoint ships — is individually cleaner and was rejected only because the
brief for this wave named all five together; a future reviewer who finds this
trade-off wrong should split the declaration back out per-wave rather than
reverting the principal-aware gate this record actually depends on.
...
`OrderAmendmentService.propose`'s own guard.
...
**Wire contract.** `OrderActionCode` (Java) and `ORDER_ACTION_CODES` (frontend,
`order-actions.ts`) both name nine values; the frontend's existing
forward-compatible rendering (`actionLabel`'s `default` case, `onActionClick`'s
`default` case) already treats an emitted-but-unhandled code as a harmless,
inert, correctly-labelled button — no frontend change was needed to add
`AMEND` to the emitted set safely.
...
nothing yet emits the four inert values.

## Implementation checklist

- [x] `OrderActionsPolicy.availableFor` takes `grantedCapabilities` and gates
      `APPROVE`/`REJECT`/`ADVANCE`/`CANCEL`/`AMEND` on it
- [x] `OperationsOrderController` computes the granted set once per request
      from `AuthorizationService` and threads it through list and detail
...
- [x] `OrderActionsPolicyTests` extended per role (`LOCATION_STAFF`,
      ...
      `LOCATION_MANAGER`) against `PlatformRole`'s own capability sets
...
fulfilment mode — proved today by `OrderActionsPolicyTests` and
`OperationsOrderControllerActionCapabilitiesTests`. A terminal order's row
always offers a working overflow menu. Fully met once `COMPLETE`, `RESOLVE`,
`ASSIGN_COURIER` and `ISSUE_INVOICE` are each emitted by a real endpoint and
covered by the same per-role proof.
