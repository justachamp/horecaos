...
- Implementation status: Partial — the alerts, the probes, the watchdog, and the runbooks are built. `uz.horecaos.platform.observability` publishes the gauges; `infra/observability/horecaos-probe.sh` evaluates every alert in the table below at its stated threshold and tier and pings the dead-man's switch; `management.endpoint.health.group` splits liveness, readiness, and the external customer probe; the platform runbooks are written. `compose.production.yaml` runs the box and `infra/production/caddy/Caddyfile` fronts it. **Collection and display are now built too**: `infra/observability/compose.observability.yaml` is a self-contained Prometheus, Alertmanager, and Grafana overlay — verified with `promtool`, `amtool`, and one real end-to-end boot rather than assumed — attached with two `docker compose -f` flags onto whichever of the two competing production compose trees is chosen, without editing either. It pages six of the table's alerts over the same Telegram channel the probe already uses and serves the single dashboard this record asks for; the rest of the table's alerts stay on the probe alone because each needs a fact a metrics scrape of the application cannot see (`infra/observability/README.md` says which and why). **Three things in this record are not built and are enumerated under "What is not built yet" below**: the off-box half (two external services), traces, and one alert that cannot be implemented as specified — dead letters by `FailureCategory` on the outbox side, which has no column to group by. **Eight checklist items remain open, not the three an earlier revision of this line counted**: the `app`/`worker` role split (one container runs every scheduler today, and the four switches this record names no longer cover them); the reverse proxy's Payme allowlist, body caps and rate limits, of which the Caddyfile carries none; the WireGuard and key-only-SSH host configuration; the laptop-loss rehearsal; the OpenBao AppRole file mounts; the restore rehearsal's money reconciliation; the cutover suppression window and ownership panel; and the external vulnerability scan
...
- [x] Stand up Prometheus, Alertmanager, and the single dashboard on the box; deliver alerts off it. — `infra/observability/compose.observability.yaml`, a self-contained overlay (its own compose file plus its own config directory) rather than an edit to either competing production compose tree, attached with two `-f` flags — verified against both `compose.production.yaml` and `deploy/compose.production.yml` — and documented in `infra/observability/README.md`. Prometheus scrapes `/actuator/prometheus` by sharing `platform-app`'s network namespace (`network_mode: "service:platform-app"`) rather than by any change to `LocalMetricsScrapeMatcher`, verified empirically rather than assumed from the Docker documentation. Alertmanager pages six of the table's alerts — order flow stalled (the outbox/inbox legs), payment callback failing, provider circuit stuck open, monetary dead letter, ownership fence burst, onboarding stalled — over the same Telegram channel the probe already uses; `promtool test rules` asserts the behaviour of each, not just its syntax. The rest of the table's alerts still evaluate on `horecaos-probe.sh` alone, because each needs a fact this Prometheus cannot see: `pg_isready`/`bao status` against another container, a stamp file on the host filesystem, a live TLS handshake, Docker's own restart counters, or Kafka consumer lag. That is by design, not an oversight — duplicating them here would mean two evaluators that can silently disagree — and the reason for each is written into `prometheus/rules/adr-0023-alerts.yml`'s own header. The dashboard is `grafana/dashboards/is-it-working.json`; everything on the "deliberately not an alert" list that this Prometheus can see is now on it, and the panel for what it cannot see says so in the dashboard itself rather than staying silent. Not yet done: wiring this overlay's `-f` flag into whichever production tree wave 55 settles on — that is a one-line addition to a deploy script or runbook once the tree is chosen, not a rebuild.
...
- [ ] Configure the reverse proxy: TLS, the Payme source allowlist, body size caps, per-binding rate limits, and no content filtering on the callback roots. — TLS and the actuator surface are in the Caddyfile; the allowlist, the caps, and the rate limits are not.
...
- [ ] Add the cutover alert, the replicator-lag signal, and the ownership panel, and define the backfill suppression window. — The cutover alert is built and counts from `horecaos.migration.writes.fenced`; it is now also evaluated by Prometheus (`HorecaosOwnershipFenceBurst`) in addition to the probe, and the dashboard graphs fenced writes by capability. The replicator-lag signal has nothing to measure. **The ownership panel is still not built**: a table of `migration.scopes` is a PostgreSQL query, not a metric, and the dashboard this wave added has only a Prometheus datasource — "the panel needs the dashboard" is no longer the blocker now that a dashboard exists, but the panel itself, and the Postgres datasource it needs, are not there yet. The suppression window is not implemented: neither the probe nor the new Alertmanager routes have a maintenance mode, so an announced backfill will currently produce digest noise on both.
- [ ] Run the external vulnerability scan before the pilot serves customers.

## What is not built yet

Three gaps, listed here rather than left to be discovered from the checklist.
A fourth — the dashboard — is closed: `infra/observability/compose.observability.yaml`
stands up Prometheus, Alertmanager, and Grafana as a self-contained overlay
that attaches, with two `docker compose -f` flags and no edit to either file,
onto whichever production compose tree is chosen; `infra/observability/README.md`
documents exactly how. It is not yet wired into either tree by default — that
is the one step left once a tree is chosen. Everything this record
deliberately excluded from paging — latency, CPU, cache hit rate, a breaker
opening rather than staying open — that a scrape of the application can see is
now on that dashboard; what it cannot see (real host disk, backup age, TLS
expiry, container restart counts, Kafka consumer lag) is named on the
dashboard itself rather than silently missing from it.
