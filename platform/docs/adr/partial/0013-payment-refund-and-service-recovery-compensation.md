...
- [x] Implement the refundable-balance reservation and the Qoida payment state machine. `PaymentAttemptStateMachine` and `PaymentIntentStatus` are built and tested; `OrderSettlementService.refund` unwinds tenders in reverse settlement order against `tenders.refunded_minor` (V0048), refuses to exceed what they settled, and is now called by `OrderRemedyService`. **It runs against a real order now.** `OrderSettlementService.plan` gained its production caller: `CheckoutSettlementStep` is step 8 of `CheckoutService.checkout` and calls `OrderSettlementPort.planSettlement` unconditionally, which `CheckoutSettlementPlanner` implements over `plan`. So a `payments.order_settlements` row exists for every order whose method this build can tender against, and `CartCheckoutAndOrderTests.anOrderPlacedTheRealWayCanBeRefunded` places an order through the production `checkout()` path and refunds it to `TenderStatus.REVERSED`.
...
- [x] Register payment providers as ADR 0026 installations; add no payments-local credential rows. The schema enforces the second half — `ck_merchant_binding_secret_is_a_reference` refuses anything but a `horecaos:…:provider_payment:…` reference and both foreign keys point at `integration.installations`/`bindings`. **The write side that was missing is now built**: `MerchantBindingController` (`POST /api/v1/operations/tenants/{tenantId}/merchant-bindings`) registers a binding in `DRAFT` and drives activate/suspend/archive through `MerchantBindingService` and `JdbcMerchantBindingStore`, under `PAYMENT_MERCHANT_BINDING_MANAGE`; `JdbcPaymentBindingResolver` still only reads an `ACTIVE` row to resolve a payment.
