...
| `integrator_id` | Identifies **us**, Qoida, as the integrating system. Reused across every brand we connect to. | Clopos |

Clopos's `brand` ≈ Qoida's brand-or-tenant; Clopos's `venue` ≈ Qoida's location.
The correspondence is close enough to be useful and loose enough to be dangerous:
nothing forces one Qoida tenant to be one Clopos brand, and §3 treats that as a
...
* **`integrator_id`** — issued by Clopos to us, once, for the integration as a
  ...
  Reused across every brand. This is a *platform-level* Qoida secret, not a
  tenant-level one.
...
* **A Qoida tenant is not necessarily one Clopos brand.** A tenant running two
  Clopos brands needs two installations; a Clopos brand spanning two Qoida brands
...
| Clopos term | What it actually is | Qoida canonical |
|---|---|---|
...
* **`GOODS` with variants: the parent is not sellable.** "If a product has
  ...
  So a parent `GOODS` maps to a Qoida product shell, and each `MODIFICATION` maps
...
* **Modifiers attach only to `DISH`.** A `GOODS` product cannot carry modifier
  groups. If Qoida's catalog has modifiers on a non-dish, that combination is not
...
ADR 0012's authority table (customer-facing price → Qoida) and ADR 0018's
deterministic pricing. Question Q3 in §12.

**Mitigating position:** ADR 0012 makes Qoida authoritative for customer-facing
...
order, and Qoida learns the outcome by asking.
...
* Auto-confirmed Qoida orders → `auto_order_accept: true`. Clopos is a recipient.
...
Qoida writes onto the Clopos receipt for the restaurant's benefit — they are **not
...
* Each product line requires `product_id`, `product_name`, `count`, `price`,
  ...
  * `product_name` and `price` are **required on the line**, meaning the caller
    restates the catalog. That is convenient for us (Qoida is authoritative for
...
2. **Resolve `UNCERTAIN` by reading**, on a bounded schedule, using the heuristic
   ...
   ordering.
3. **Send `order_number` anyway** (carrying the Qoida order reference, ≤20 chars),
