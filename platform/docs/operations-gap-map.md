...
|---|---|---|---|---|---|
| **P** — pilot | 80 | 54 | 16 | 1 | **151** |
| **2** — parity | 39 | 33 | 25 | 3 | **100** |
| **3** — tail | 8 | 9 | 11 | 2 | **30** |
| **?** — no IA row | 3 | 0 | 4 | 0 | **7** |
| **Total** | **130** | **96** | **56** | **6** | **288** |
...
130 rows of 288 are finished. The pilot tier alone carries 70 rows that are neither
built nor blocked, and **54 of them are PARTIAL** — a real screen against a real endpoint
...
| `1.3 †` | New order — the call-centre order-entry screen | P | PARTIAL | A three-pane composer (customer, menu/basket, order) now exists at `/orders/new` and an operator can take a pickup, cash order end to end — but it is PICKUP-only and cash-only: delivery, promo code, pre-order time and non-cash payment are P14's rows (`1.3b`/`1.3d`/`1.3e`), still unbuilt. | XL | — | P13 | NOT BUILT  |
| `1.3a` | New order — customer pane (phone lookup, auto-create, order-history peek) | P | PARTIAL | Phone lookup, candidate list, select-to-attach and create-on-miss are built and tested — but LOCATION_STAFF/LOCATION_MANAGER lack CUSTOMER_MANAGE, so the create-customer call 403s for the exact operator role this screen targets; history peek is limited to lastOrderAt/recentOrderCount from the lookup response (itemized last-5-orders and a working «Повторить» wait for P14's row `1.3f`). | M | — | P13 |  |
...
16 rows — 8 built · 4 partial · 3 not built · 1 blocked
...
25 rows — 11 built · 5 partial · 7 not built · 2 blocked
...
| `4.2g †` | Per-item sale schedule and kitchen department on the product | P | PARTIAL | Kitchen department routing is wired to the existing routing-rules endpoint (a station-role picker on the BASIC tab, `CapacityApi.route`) — but `KitchenStationController` has no GET for an existing rule, so the picker cannot show a product's current routing and a second save 409s. The per-item sale schedule has its binding table (`catalog.item_sale_windows`, V0301), a `q-schedule-grid` editor (SCHEDULE tab, whole-set PUT, default variant only), and `CatalogAuthoringService.isOnSaleNow` correctly resolves it in the location's own timezone — but `isOnSaleNow` has no caller anywhere in ordering/checkout, so an item outside its sale window is not actually blocked from being ordered. Held at PARTIAL for that reason: the editor is built, enforcement at checkout is not wired. | L | — | P47 | NOT BUILT |
...
| `7.1a` | Business overview — sales funnel by status with cancel reasons and late completions | P | PARTIAL | `insertOrderFact` now copies `stock_disposition` and `liability_party`, so the cancellation cost (write-off vs returned stock, who bore it) is shown, and the reason code resolves to its `internal_name` instead of a machine code. Still a table, not the funnel the IA names — no funnel chart primitive exists in the design system. | S | — | P27 |  |
...
| `7.2c` | Order reports — daily summary reports 1 and 2 («Сводка») | P | PARTIAL | The ADR 0038 `LEGAL_ENTITY`-grouping bug that could throw `CombinedEntityTotalException` on a two-entity tenant is fixed (money-safe now). Still the flat table from the prior wave, not the true branch×channel grid, and neither summary carries the delivery-inclusive sum column Delever's report 1 has. | S | — | P27 |  |
...
36 rows — 16 built · 18 partial · 2 not built
...
| `10.9d` | Notifications — payment-link auto-send and aggregator shift open/close notifications | P | PARTIAL | Automation-card switches for payment-link auto-send and aggregator shift open/close notifications now exist, backed by two registered `ordering.notifications` keys — but neither is enforced: no trigger anywhere reads either key yet, so toggling a switch changes nothing in production. | M | — | P36 |  |
