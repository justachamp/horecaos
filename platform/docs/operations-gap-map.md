...
|---|---|---|---|---|---|
| **P** — pilot | 9 | 95 | 46 | 1 | **151** |
| **2** — parity | 1 | 42 | 54 | 3 | **100** |
| **3** — tail | 1 | 13 | 14 | 2 | **30** |
| **?** — no IA row | 1 | 2 | 4 | 0 | **7** |
| **Total** | **12** | **152** | **118** | **6** | **288** |

Twelve rows of 288 are finished. The pilot tier alone carries 141 rows that are neither
built nor blocked, and **95 of them are PARTIAL** — a real screen against a real endpoint
with one named capability missing. That ratio is the single most useful fact in this
document: the pilot is not a greenfield build, it is a finishing job, and most of the
finishing is frontend wiring over endpoints that already answer.
...
12 rows — 6 partial · 6 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `0.1` | Live board | P | PARTIAL | A shift supervisor gets a real live board but not the one 0.1 specifies: the operator leaderboard is a locked note, the 'Отменено' counter is a lifetime total rather than a period figure, and the whole thing renders in the operator console shell at desk distance, so it cannot be put on a TV over the pass. | L | — | P15 |  |
| `0.1a` | Oversized counters + the canonical "in progress" grouping | P | PARTIAL | The 'Отменено' counter counts every cancelled, rejected and expired order the branch has ever had, so a supervisor cannot see how many orders were cancelled today or this shift — the number only ever grows and is unreadable as a live signal. | M | — | P15 |  |
| `0.1b` | Live source-mix and type-mix | P | PARTIAL | Above ~200 concurrent orders the mix silently under-counts, and the operator gets raw channel codes and a bar made of a coloured span instead of the chart 0.1 assumes; no one can tell the board is approximating. | M | — | P15 |  |
| `0.1c` | Branch leaderboard / per-branch active-order load | P | PARTIAL | An operator taking a call cannot see which branch is swamped at the moment of choosing one, because order entry itself is a not-built placeholder; and on a multi-branch tenant the board fires one counts query per branch every 10 seconds. | M | — | P15 |  |
...
| `0.1e †` | Wallboard presentation of 0.1 (TV-distance shell + WallboardTile) | 2 | PARTIAL | The board cannot be put on a screen above the pass: at TV distance the counters, the mix bars and the branch table are unreadable, so 0.1's stated purpose — a wall-board for the shift supervisor — is served only as a desk page. | L | — | T23 | NOT BUILT |
...
40 rows — 2 built · 23 partial · 15 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `1.1` | Order board — queue shell, table and columns | P | PARTIAL | An operator cannot see who the customer is, which courier holds the order, how it will be paid, or the delivery fee without opening every row one at a time; branch is absent because the board is hard-scoped to one location at a time via CurrentLocation. | L | — | P04 |  |
...
| `1.1d` | Search across per-provider external IDs | P | NOT BUILT | When Yandex Eats or Uzum calls about order 77-A-3391, the operator has no way to find it: neither the aggregator id, nor the customer phone, nor the HorecaOS order number is searchable from the board. | M | — | P04 |  |
| `1.1e †` | Row action menu (inline + overflow, server-driven) | P | PARTIAL | Only four action codes exist. Delever's "…" menu opens roughly twelve dialogs; the other eight (courier, POS, payment, fiscal, amend, print) have no action code to render. | S | — | P05 | BUILT |
...
| `1.2a †` | Server-supplied actions[] capability array | P | PARTIAL | `actions[]` is a status array wearing a capability array's name. `OrderActionsPolicy.availableFor(OrderStatus, FulfillmentMode)` (platform/.../ordering/application/OrderActionsPolicy.java:32) takes no principal and consults no grant, so `LOCATION_STAFF` — which holds ORDER_READ/APPROVE/ADVANCE and not ORDER_CANCEL — is offered «Отменить» on every open order and is refused at OperationsOrderController:472. SUPPORT_AGENT, TENANT_FINANCE, BRAND_MANAGER and COURIER_DISPATCHER get the same false offer. The array is also closed at four codes while the controller serves completion, amendment, amendment confirmation and bulk actions with no code that reaches them. | S | — | P05 | BUILT |
...
15 rows — 10 partial · 5 not built
...
| `3.2` | Live map | 2 | PARTIAL | A dispatcher cannot see where anyone is at a glance — positions render as decimal coordinates in a table. No branch filter, no partner-courier pins (ADR 0014 partner couriers carry no position at all), and the audited track-reveal path is defined in core/api/operations-paths.ts:291 but called by nothing, so stored tracks cannot be opened for a dispute. | L | No map primitive exists in the app (PART 4 pilot blockers: MapCanvas/MapPin); the map provider decision (Yandex Maps) and its key are not made anywhere in the repo. | T03 |  |
| `3.3` | Couriers (in-house roster) | P | PARTIAL | A manager cannot keep the compliance file the self-employment model assumes: no passport, PINFL, driving licence, vehicle registration/plate/fuel, photo, emergency contact, courier groups or branch bindings (a courier is attached to a branch only by an open shift), and no online status or rating. Courier-app access is not provisioned here — the register form asks the operator to type a Keycloak subject created outside the console. | L | — | P19 |  |
| `3.4a` | Courier types (Тип курьера) | 2 | PARTIAL | Two of the three attributes the IA names have no column and no field: the 'starting minute' (the minute from which a type begins earning) and work mode. The create form also never sends maxDistanceMeters, so a type's upper distance bound can only be set outside the console. | S | — | T16 |  |
| `3.4b` | Courier rate cards (Тариф курьера) | 2 | PARTIAL | The per-km salary model — one of the three the IA says must be expressible — cannot be authored: PER_KM_BAND's distance ladder has no editor. Worse, ADR 0042's own status line states nothing in production calls CourierAccrualService.recordDelivery, so an activated card earns a courier nothing outside a test; a manager can publish pay terms that the ledger never applies. | L | — | T16 |  |
| `3.4c` | Bonus / penalty rule definitions | 2 | NOT BUILT | A manager cannot define 'late delivery → −5,000' or 'twenty deliveries in a shift → +50,000' once and have it apply. Every bonus and penalty has to be keyed in per courier by hand, each one through the four-eyes approval path. | L | — | T16 |  |
| `3.5` | Shifts & attendance (Посещаемость) | 2 | PARTIAL | There is no roster: only the shift a courier opened himself is visible, so a manager cannot plan who works when or compare planned against actual. The 'check courier work schedule' authorization gate exists in the backend but cannot be switched on or off from the console because the policy screen (3.9) is read-only. Cash handed over at shift close is confirmed in Finance (features/finance/cash/cash-reconciliation-page.ts), not here. | L | — | T17 |  |
| `3.6` | Delivery zones | P | PARTIAL | Circles only — no polygon drawing and no map at all, so a real city zone with a river or a ring road in it cannot be drawn. `delivery-zones-page.ts` `submitDraft` (:201-209) never sends `deliveryTariffId` although `delivery-zones-api.ts:115` accepts it, so every console-drawn zone carries a null tariff and the zone-beats-branch tariff precedence the IA requires cannot be exercised. The zone → branch set is half-reachable: `OperationsServiceZoneController` binds locations to a zone (`:182`) and there is no unbind and no list of what a zone is bound to, so a wrong binding is permanent. No zone → courier-service binding exists on either side. | L | PolygonEditor/MapCanvas do not exist (PART 4 pilot blockers) and the map provider is unchosen. | P20 |  |
| `3.6b` | Regions & geocoder bounding boxes | P | NOT BUILT | An operator cannot create a region or set the SW/NE bounding box that constrains the geocoder and catches a transposed latitude; a region can only be inserted by SQL, so onboarding a merchant in a second city needs a developer. | M | — | P20 |  |
| `3.6c` | Bulk geozone upload | 2 | NOT BUILT | Migrating a merchant who already has dozens of zones means re-drawing each one by hand in the console. Not pilot-critical (one branch draws a handful of circles), but it is what a Delever migration needs. | L | — | T17 |  |
| `3.6d` | Free geozone (Бесплатная геозона) | P | PARTIAL | The mechanism exists and the console cannot reach it: because `submitDraft` never sends `deliveryTariffId` (`3.6`), no console-drawn zone can be bound to the zero-resolving tariff that *is* a free geozone, and nothing on the zone list says which zones are free. This row is discharged by `3.6`'s tariff binding plus a zero-fee label on the zone list — not by a third geometry layer, which ADR 0037 refuses. | S | — | P20 |  |
| `3.7` | Delivery tariffs | P | PARTIAL | An operator can price exactly one flat band: no distance tiers, no peak-hour surcharge windows, no free-delivery or discount rules on the tariff, no provider-quote-vs-own-tariff toggle, and no ROAD mode (ADR 0037 records RoadDistancePort answering empty, so ROAD would silently price as radius anyway). Nothing in the UI binds a tariff to a branch, so every location rides the brand default. | L | — | P20 |  |
...
| `3.9` | Courier policy | 2 | PARTIAL | Nothing on this screen can be changed — shift enforcement, cash ceiling, penalty-approval threshold and the settlement/reverification windows are read-only, so an operations manager must file a ticket to change them. Six of the switches the spec names (GPS accept/action radius, show-only-kitchen-ready, reveal-customer-location timing, telemetry collection-gate default, post-delivery payment check, courier billing mode) have no field in any policy document under ADR 0042 or ADR 0045. | L | — | T03 |  |

## §4 — Catalog

25 rows — 11 partial · 12 not built · 2 blocked
...
| `4.4d` | Catalog base settings (use stock logic; QR/kiosk take hall prices) | P | NOT BUILT | A merchant cannot turn quantity tracking on for the whole company in one place — the only expression is per variant per location, which is unusable on a 600-item catalogue — and cannot say «QR and kiosk sell at hall prices», so the two channels either need a full price plane authored by hand or silently take the wrong plane. Both are tenant-wide switches in the parity product and both are what makes the per-item plumbing in `4.4`/`4.4b` affordable. | M | — | P46 |  |
...
19 rows — 4 built · 10 partial · 4 not built · 1 blocked
...
| `5.2a` | Contact points on the customer record | P | PARTIAL | An operator who mistypes a phone number cannot correct or remove it, cannot add an email at all, cannot change which number is primary, and cannot see whether a number is verified — which is what decides whether marketing may reach it (MarketingEligibility.NO_VERIFIED_ENDPOINT). | M | — | P25 |  |
| `5.2b` | Consent state and marketing eligibility, separated from record existence | P | PARTIAL | Recording consent from this screen cannot make a customer contactable: the console omits brandId (so the row is tenant-wide and never matches a brand-scoped check) and the purpose is free text an operator must guess against three different strings already in use. The screen also never states the eligibility answer, so an operator cannot tell an imported or operator-created customer's non-contactable state from a contactable one, and the policy version recorded as legal evidence is a placeholder literal. | M | There is no tenant-wide consent-type/policy-version registry anywhere (settings/data-privacy/data-privacy-page.ts says so in its own doc: 'a decision log exists, a type registry does not'); the purpose vocabulary is an ADR 0015/0044 decision, not a screen fix. | P25 |  |
| `5.2c` | Saved addresses, operator-visible and editable | P | PARTIAL | Every address an operator saves is created without coordinates, so a phone order captured here cannot be zone-resolved, tariffed or handed to a courier by location — only by the free-text landmark. There is no suggest, no pin, no map. | M | Named open input on ADR 0015: geocoder/map provider selection (product). PART 4 lists MapCanvas + MapPin/AddressPicker as a pilot blocker with 'no map primitive exists at all'. | P25 |  |
...
11 rows — 9 partial · 1 not built · 1 blocked

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `8.1` | Payments & settlements | P | PARTIAL | An operator cannot find a payment without already knowing the order's UUID — the lookup box takes a raw id and there is no lookup by public order number and no payment-status column on the order queue (OperationsOrderController's responses carry no payment_status_projection field, stated in payments-page.ts's own doc). A void still has no provider-mappable cancellation reason — a refund's reasonCode is free text the operator types — and order status / intent status render as raw enum tokens rather than localized labels. | M | — | P29 |  |
| `X.1 †` | 8.1 — split tender: the payment[] array (cash + cashback + deposit) | 3 | PARTIAL | An operator cannot see or record an order paid partly in cash and partly from cashback or a deposit — one order has one intent and one method, so a mixed settlement simply cannot be represented, let alone refunded per tender. | XL | — | W05 | NOT BUILT |
| `8.2` | Fiscal receipts | P | PARTIAL | An operator can work the blocked queue but cannot ask 'what is this one order's fiscal status' from the console — the per-order endpoint exists and the screen never calls it — and every row identifies its order by UUID, so the queue cannot be tied back to an order number without a second tool. | M | — | P29 |  |
...
| `8.6` | Subscription & billing | 2 | PARTIAL | A merchant can see what plan it is on and cannot do anything about it: no purchasable modules with inline purchase (the catalogue is CommercialModuleController at /api/v1/control-plane/modules :56 and /api/v1/control-plane/tenants/{id}/modules :122 — control-plane only), no arrears state or restricted-feature banner (ArrearsController is /api/v1/control-plane/arrears :49 only), and no credit-expiry warning. | L | — | T19 |  |
| `X.4 †` | 8.6 — invoices the merchant can read | 2 | PARTIAL | A merchant cannot see its own HorecaOS invoices at all — every statement read is control-plane-scoped, so the tenant has to ask platform staff for a copy — and an issued statement reaches an accountant only as CSV pulled by platform staff, never through an e-invoicing operator. | M | — | T19 | NOT BUILT |
...
20 rows — 1 built · 8 partial · 11 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `9.1 †` | Users & roles — accounts and role assignment (grants) | P | PARTIAL | A branch manager cannot open Staff at all: every grant endpoint demands IAM_GRANT_MANAGE at TENANT scope and only tenant-owner and tenant-admin carry it (iam/api/PlatformRole.java:284, :421), so the spec's «Chilonzor manager sees Chilonzor's team» view is unreachable by the person it was designed for. Bulk assignment is N client calls with client-composed partial-failure reporting (spec §11.8). | S | — | P30 | BUILT |
...
| `9.1c †` | Users & roles — permission-gated navigation | P | PARTIAL | Every operator sees all fourteen rail entries — `frontend/operations/src/app/shell/navigation.ts` declares fourteen `path:` items across three groups (service, people, business) — and discovers what she may not do by clicking into a screen and being refused, instead of the legacy dashboard's behaviour of hiding a section she has no rights to. `auth.guard.ts` asks only whether anybody is signed in. | M | — | P30 | NOT BUILT |
| `9.1d †` | Users & roles — locked-by-plan vs denied-by-permission, with inline upsell | P | PARTIAL | When a screen refuses, the operator cannot tell whether her plan does not include the feature (a purchase away) or her job does not permit it (a wall) — and there is nowhere to buy the module inline. | M | — | P30 | NOT BUILT |
...
| `9.2d †` | People — created-by / accepted-by attribution | 2 | NOT BUILT | A manager cannot see how many orders an operator took today from the person's own card, and the actor on an order is still a UUID rather than a name. | M | — | T08 | PARTIAL |
| `9.3` | Activity & audit — the activity log screen (История изменений) | 2 | PARTIAL | Beyond 200 events in the chosen window the operator simply cannot page further, and «what happened» is a dotted code like `iam.grants.revoke` rather than a sentence, so the log reads as machine output. | M | — | T08 |  |
| `9.3a` | Activity & audit — field-level before/after diff | 2 | PARTIAL | For almost every action the log says what the value became, never what it was, so «who changed the minimum order sum from 30 000 to 50 000» cannot be answered from the screen. | L | — | T08 |  |
| `9.3b` | Activity & audit — a named human actor, even for background paths | 2 | NOT BUILT | The most valuable screen in the section is unreadable: an incident review shows a column of UUIDs, and no operator can tell which of them is the shift manager. | M | — | T08 |  |
| `9.3c †` | Activity & audit — a bulk action produces N records, not one | 2 | NOT BUILT | A manager reading the log sees twelve separate rows from one bulk suspension with no visual sign they were one action, and has to copy a correlation id into a text box by hand to group them. | S | — | T08 | PARTIAL |
| `9.4` | Approvals — the maker-checker worklist | 3 | PARTIAL | Two of the IA's four named cases cannot appear: no call site raises a PII-export approval (CustomerController.export records the reveal and does not gate it) and there is no discretionary order-line discount producer. A manager also cannot see what she approved last week, and thresholds are authored only in control-plane (ApprovalPolicyController is /api/v1/control-plane/... only). | M | — | W03 |  |
...
| `X.4` | Проверка доступа — access check («can she do this, and why») | 3 | NOT BUILT | When an operator says «it says I can't», the manager has no way to see whether a job is missing, the scope is wrong or the grant lapsed — she can only guess from the Доступ tab. | M | — | W03 |  |
...
36 rows — 2 built · 18 partial · 16 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `10.0` | Settings home & "Find a setting" | P | PARTIAL | An operator opening Settings cannot tell whether this restaurant is actually able to trade — no readiness checks, and no way to search for a setting by name; they must know which of the twelve doors owns the field they want. | M | — | P31 |  |
| `X.1` | Scope bar + InheritedField (settings.md §1.1/§1.2 — the pattern every row renders through) | P | NOT BUILT | A merchant cannot set anything at brand level and override it for one branch, cannot see where a value came from, and cannot tell which level they are about to change — the single worst settings mistake the spec names. Every settings screen writes one fixed scope. | XL | — | P31 |  |
...
| `10.3b` | Order policy — Cards 2–5 (timings & SLA, automation/auto-accept, conditions, operator order entry) | P | NOT BUILT | An operator cannot set VAT, a minimum order sum, the late threshold that colours the order queue, the working-day boundary, auto-accept rules, out-of-zone rejection, or what operators may do when hand-entering an order — none of these knobs exists anywhere in the product. | XL | — | P46 |  |
...
| `10.7a †` | Fiscalization — Tab 1 Юридические лица (legal entities, INN, per-branch assignment) | P | PARTIAL | Working, with two rough edges: the app still calls the control-plane path although the operations mirror exists (an ADR 0031 surface violation), and suspend/archive have no HTTP surface on either controller, so a retired company can only be left ACTIVE. | S | — | P34 | BUILT |
| `10.7b` | Fiscalization — Tab 2 Фискальные терминалы | P | NOT BUILT | An operator cannot register the branch's fiscal terminal, see whether it is alive, or say which terminal a cash or courier tender prints on — so TERMINAL-responsibility methods can never be validated at activation. | L | — | P34 |  |
| `10.7c` | Fiscalization — Tab 3 Классификация товаров (IKPU/MXIK coverage, delivery-line IKPU + package code, marking, VAT defaults) | P | NOT BUILT | An operator cannot see how much of the menu is unclassified, cannot backfill IKPU/package codes in bulk, and cannot set the delivery line's own IKPU or turn marking on — a legal blocker that surfaces at a tax audit rather than at configuration. | L | — | P34 |  |
| `10.8a` | Integrations — hub: provider installs, connect fields, secret door, rotation, bindings | P | PARTIAL | An operator who signed in today cannot see the bindings created in an earlier session (no list endpoint), cannot retire an installation, and cannot express the per-branch install model with tenant-default → branch-override that the IA and ADR 0030 both assume — an install is tenant-wide. | M | — | P35 |  |
...
| `10.8c †` | Integrations — health & errors (last successful inbound per branch × provider, error taxonomy, merchant-scoped replay) | P | PARTIAL | A merchant cannot see that their aggregator stopped pushing orders two hours ago, cannot read an operator-legible reason, and cannot replay their own failed inbound message — they have to phone HorecaOS. | L | — | P35 | NOT BUILT |
| `10.8d` | Integrations — partner API credentials as an OAuth client with a rotatable secret | P | NOT BUILT | A tenant cannot issue an API client to an aggregator or rotate its secret; the credential can only be created by hand in the database, which is exactly the base64(login:password) handover the IA says HorecaOS replaces. | M | — | P35 |  |
| `10.8e` | Analytics installs (GTM, GA4, Search Console), the GA4 ecommerce event contract, and the telephony provider install | P | NOT BUILT | A merchant cannot install its own GTM container, GA4 property or Search Console verification, and no purchase, add-to-cart or begin-checkout event is emitted for any of them — so every tenant's marketing spend is unattributable and the platform's single hard-coded counter is both switched off and, if switched on, the wrong tenant's. The event contract is the larger half: a named, versioned ecommerce event set the storefront emits and the aggregators do not, which is a producer, not a settings row. Telephony needs only the install hub. | L | — | P35 |  |
...
| `10.10a †` | Reference data — cancellation and completion reasons | P | PARTIAL | Works, with two named gaps: reasons cannot be drag-reordered (no display_order column), and the screen reaches across to the control-plane surface, so this row is an ADR 0031 violation waiting to be moved. | S | — | P37 | BUILT |
| `10.10b` | Reference data — business calendar (holidays, business-day boundary across midnight, weekend definition) | P | NOT BUILT | A tenant cannot enter its own holidays or movable Islamic dates, cannot declare its weekend, and cannot set a business day that closes after midnight — so every report cuts the day at the platform's assumption. | L | — | P37 |  |
| `10.10c †` | Reference data — SLA bucket boundaries | 2 | NOT BUILT | An operator cannot change where 'late' starts. Nothing can be built until someone decides whether tenant-configurable buckets are allowed at all and, if so, how historical comparability survives a change — the code and the IA currently promise opposite things. | M | Owner/product decision: ADR 0043 fixes buckets per release (SlaBucketController is deliberately read-only) while the IA promises tenant-configurable buckets; one of the two has to be withdrawn. | P37 | BLOCKED |
| `10.10d` | Reference data — branch tags | 2 | NOT BUILT | A chain cannot tag branches (24/7, has parking, airport) and therefore cannot filter or group by tag anywhere in the console. | M | — | P37 |  |
| `10.11` | Data & privacy (retention schedules, consent definitions, DSAR/erasure, export audit) | 3 | PARTIAL | A merchant can see who revealed personal data but cannot see or set their own retention periods, cannot read a consent-type registry, and cannot raise or answer a data-subject export/erasure request from the console. The page's own retention text is now stale against ADR 0092's sweepers. | L | — | W03 |  |
...
| `X.3 †` | HorecaOS support visits (who from support entered this account, and ending a live visit) | 2 | PARTIAL | Nothing functional. It is the one Settings screen with no spec file of its own (no support-visits-page.spec.ts beside it), so it is the least test-covered page in the section. | S | — | P31 | BUILT |

## §X — Shells and design system

40 rows — 30 partial · 10 not built
...
| `X.3` | Wallboard shell (TV-distance, oversized counters) | 2 | NOT BUILT | There is nothing a supervisor can put on a screen above the pass: the board is readable at desk distance only, still spends a rail and a topbar on navigation nobody at the wall can use, has no fullscreen or unattended mode, and no visible refresh/staleness stamp, so a frozen wallboard is indistinguishable from a quiet shift. | M | — | T23 |  |
...
| `X.5` | LocalizedFieldGroup (language tabs, default marker, completeness) | P | PARTIAL | On roughly forty forms — brand profile, categories, menus, channels, campaigns, promo codes, reference data — a tenant edits whatever language they happen to be in, with no default-language marker and no completeness indicator, so nobody can tell which of ru/uz/en is empty before publishing. IA PART 4 also asks the group for an **optional auto-translate** button and the parity matrix grades trilingual content with auto-translation ● — and no machine-translation provider exists anywhere on either side (`ProviderCategory` has POS, PAYMENT, DELIVERY, MARKETPLACE, NOTIFICATION, GEOCODING and VOICE and no translation category; every `translate` hit in platform/src/main/java is the manual `PUT /translations` path), so the button has no service to call. The component ships without it and the provider decision is deferred in PART B. | M | — | P02 |  |
| `X.6` | DataGrid (inline edit, fill-down, keyboard nav, virtualization) | P | NOT BUILT | Nobody can backfill IKPU/package codes across hundreds of products or re-price a menu in one pass: every price, every availability flag and every fiscal code is one row, one form, one request. | L | — | P03 |  |
| `X.7` | MatrixGrid (editable cross-tab with row/column bulk toggle) | P | PARTIAL | There is no cross-tab: turning cash off for six channels is six trips through a row picker, nothing bulk-toggles a row or a column, and a channel that cannot fiscalise a method shows an ordinary empty checkbox rather than an unavailable cell. | M | — | P03 |  |
| `X.8` | ActionMenu + Drawer + Modal + ConfirmDialog | P | PARTIAL | A keyboard operator cannot dismiss any dialog with Escape and can Tab straight out of it into the page behind; the queue's own rows carry no overflow menu at all, so every order action costs a trip through the detail pane; each new dialog is copy-pasted markup that can drift. | M | — | P01 |  |
| `X.9 †` | Combobox (async search, create-on-miss, multi-select chips) | P | PARTIAL | An operator cannot type a phone number and get the customer back with create-on-miss, nor search a product catalogue of thousands — the call-centre screen's unknown-caller card is the only create-from-a-phone-number path in the console. | M | — | P02 | NOT BUILT |
| `X.10 †` | MoneyInput / PercentInput / MoneyOrPercent | P | PARTIAL | Somebody typing a tariff sees an unformatted spinner: no thousands grouping, no som suffix, no guard against a pasted '125 000' or a decimal point, and discount fields that must accept either a sum or a percent have no control that expresses the choice. | S | — | P02 | NOT BUILT |
| `X.11` | DateRangePicker + TimeInput + DayOfWeekToggle + ScheduleGrid | P | PARTIAL | A merchant cannot change their own opening or delivery hours from the console at all — only read them — and no report can be run over an arbitrary range, so 'last Friday vs this Friday' is unanswerable. | L | — | P02 |  |
...
| `X.14` | SecretInput (masked, reveal-once, copy, rotate, last-used) | P | PARTIAL | After rotating a Click or Payme secret an operator gets no evidence of what is now stored, when it was last used, or which binding it belongs to — and any binding made in an earlier session is invisible until the row-listing endpoint exists. | S | — | P35 |  |
| `X.15` | StatusPill extensions (lateness overlay; dual-state order+cooking pill) | P | PARTIAL | Status and lateness read as two unrelated visual systems from screen to screen, and the order board shows no cooking state beside the commercial status — an operator has to open the kitchen screen to learn whether a late order is even being cooked. | S | — | P01 |  |
| `X.16` | LockedState + DeniedState (distinct from EmptyState) | P | PARTIAL | A denied operator gets a different sentence on every screen and no route to whoever could grant the capability; a plan-locked feature looks identical to a broken one and offers no way to buy it. | S | — | P01 |  |
| `X.17 †` | Toast + InlineAlert | P | PARTIAL | A successful save is announced nowhere the operator is looking, and nothing is announced to a screen reader at all; an action taken inside a dialog that closes on success gives no confirmation that it happened. | M | — | P01 | NOT BUILT |
| `X.18` | DataTable extensions (saved views, persisted filters, column chooser, server paging + infinite scroll, selection with bulk-action bar, row action menu) | P | PARTIAL | An operator's per-tab filters are lost on every navigation and every reload — the legacy dashboard persisted them and merchants will notice — and outside the stop list nothing can be selected and acted on in bulk or have its columns chosen. | L | — | P03 |  |
| `X.19` | Chart family (line, bar, stacked, donut, funnel, histogram, cohort heatmap, sparkline, geo heatmap) | 2 | PARTIAL | A manager cannot see a trend, an ABC cumulative curve or a funnel anywhere — every report answers in numbers and tables, so 'is today going normally, and if not where' has to be read line by line. | L | — | T09 |  |
| `X.20` | KpiTile (value + delta + sparkline) and WallboardTile | 2 | PARTIAL | The same average-check tile is implemented twice in two files with no shared definition of the delta, no tile carries a trend, and the wallboard counter is a font size rather than a component with TV-distance rules. | M | — | T09 |  |
...
| `X.25` | ConditionBuilder + RuleList with priority reorder + RuleSimulator | 2 | PARTIAL | A marketer cannot author a promotion, an operator cannot express a provider fallback rule, and nothing can be dry-run before it goes live — segment predicates are the only rules anybody in this console can write. | L | — | T21 |  |
| `X.26` | Timeline + DiffViewer + ActorChip | 2 | PARTIAL | Most audit rows name a UUID rather than a person and describe themselves in machine codes; an order's production and delivery history is invisible; and the same three ideas are re-implemented per screen. | M | — | T22 |  |
...
| `X.28` | PhoneFrame siblings (AggregatorCardFrame, KioskFrame 9:16, TelegramMiniAppFrame) | 2 | NOT BUILT | Nobody can see how a product card will look on an aggregator, on a kiosk, or inside a Telegram Mini App before publishing it — the preview affordance the IA counts on is unbuilt down to its base component. | M | — | T22 |  |
| `X.29` | NumberStepper | 2 | NOT BUILT | Adjusting a quantity on a touch screen means selecting text inside a spinner; modifier min/max and portion bands have no control that shows their limits. | S | — | P02 |  |
| `X.30` | SplitPane (master-detail) | 2 | PARTIAL | The split is not resizable and not remembered, and five sections each own a copy of the same CSS and the same media query, so narrow-screen behaviour differs per section. | S | — | P01 |  |
| `X.31` | RichTextEditor (block-based, sanitized) | 2 | NOT BUILT | A merchant cannot write a static page, a news item or a vacancy body at all — and since the parity behaviour being replaced is Delever accepting raw tenant HTML, this has to arrive as a sanitized block editor, not a textarea. | L | — | T22 |  |
| `X.32` | ColorInput | 2 | NOT BUILT | A tenant cannot choose its late-highlight colour or a per-channel brand colour, so every merchant's console looks identical and the accessibility question the ramp raises never gets asked. | S | — | P02 |  |
| `X.33` | Steps / ProgressRail | 2 | NOT BUILT | An operator infers where an order sits in its lifecycle by reading a status word and scanning past events, instead of seeing the position; a long-running import would have nowhere to show its stage. | S | — | T22 |  |
...
| `X.38` | OtpInput (staff MFA) | 3 | NOT BUILT | Staff cannot be given a second factor because there is nowhere to enter one — a shared manager account on a restaurant PC is protected by a password alone. | S | — | T22 |  |
...
| `X.40` | dir/script-safe type stack (uz-Latn, Cyrillic ru, Latin en; kk and ka on the roadmap) | 2 | PARTIAL | No Kazakh or Georgian face is bundled and no `dir` handling exists, so adding kk or ka means a font and layout change rather than a message file; and the type scale is already drifting — feature stylesheets set raw px font sizes that the closed scale forbids, with nothing checking it (no ESLint or token drift check in this app, per ADR 0035's status line). | S | — | T22 |  |
...
| **P34** | `V0247`–`V0249` | `fiscal.fiscal_terminals`; legal-entity short name, VAT certificate, tax profile, registered address, contact phone |
| **P35** | `V0250`–`V0252` | partner API client last-used; provider-installation last-used watermarks |
| **P36** | `V0253`–`V0255` | the per-version variables schema; the template unique index widened past `(tenant, brand, key, channel)` |
| **P37** | `V0256`–`V0258` | branch tags and their location assignment; the tenant business calendar and its boundary |
| **T16** | `V0259`–`V0261` | courier-type starting minute and work mode; the adjustment-reason amount and condition columns |
| **T17** | `V0262`–`V0264` | `courier_roster_entries` |
...
| Wave | Title | Tier | Size | Points | Rows | After |
|---|---|---|---|---|---|---|
| **P01** | Console primitives: overlays, feedback, status | P | 1d | 7 | `X.8`, `X.30`, `X.17`, `X.16`, `X.15` | — |
| **P02** | Console primitives: value, time and text inputs | P | 2d | 11 | `X.10`, `X.11`, `X.29`, `X.32`, `X.9`, `X/X.5` | P01 |
| **P03** | Console primitives: DataGrid, DataTable extensions, MatrixGrid | P | 2d | 10 | `X.6`, `X.18`, `X.7` | P01, P04 |
| **P04** | Order list read model and query surface | P | 1d | 6 | `1.1`, `1.1d` | — |
| **P05** | Server-driven actions: actions[] as a capability array | P | 1d | 2 | `1.2a`, `1.1e` | P01, P04 |
...
| **P15** | Live board: period scoping, mixes, branch load | P | 2d | 10 | `0.1`, `0.1a`, `0.1b`, `0.1c` | — |
...
| **P19** | Courier roster: the compliance file | P | 1d | 4 | `3.3` | — |
| **P20** | Zones, regions, free geozones and delivery tariffs | P | 2d | 11 | `3.6`, `3.6d`, `3.6b`, `3.7` | — |
...
| **P25** | Customer record depth: contact, consent, address | P | 2d | 6 | `5.2a`, `5.2b`, `5.2c` | — |
...
| **P29** | Finance: payments and fiscal, per order | P | 1d | 4 | `8.1`, `8.2` | — |
| **P39** | Payment mix and the tender fact | P | 2d | 4 | `7.1c` | P33 |
| **P30** | Staff: gated navigation, locked versus denied | P | 1d | 5 | `9.1`, `9.1c`, `9.1d` | — |
| **P31** | Settings: the scope bar, readiness and support visits | P | 2d | 11 | `10/X.1`, `10.0`, `10/X.3` | — |
| **P46** | Settings: order policy cards and catalog base settings | P | 2d | 10 | `10.3b`, `4.4d` | P31 |
...
| **P34** | Settings: fiscalization | P | 2d | 9 | `10.7a`, `10.7b`, `10.7c` | P31 |
| **P35** | Settings: integrations, analytics and partner credentials | P | 2d | 13 | `10.8a`, `10.8c`, `10.8d`, `10.8e`, `X.14` | P31 |
| **P36** | Settings: notifications | P | 2d | 13 | `10.9a`, `10.9b`, `10.9c`, `10.9d`, `X.27` | P02, P31 |
| **P37** | Settings: reference data | P | 2d | 9 | `10.10a`, `10.10b`, `10.10c`, `10.10d` | P31 |
...
| **T23** | The wallboard shell | 2 | 1d | 6 | `0.1e`, `X/X.3` | P15 |
| **T02** | Kitchen buffer, expo gate, VDU and capacity | 2 | 2d | 7 | `2.2`, `2.3`, `2.4`, `2.6` | P16 |
| **T03** | Live map and courier policy | 2 | 1d | 8 | `3.2`, `3.9` | P19 |
| **T16** | Courier types, rate cards and adjustment rules | 2 | 2d | 9 | `3.4a`, `3.4b`, `3.4c` | P19 |
| **T17** | Shifts, attendance and bulk geozone upload | 2 | 1d | 8 | `3.5`, `3.6c` | P19 |
...
| **T19** | Finance: subscription and invoices | 2 | 1d | 6 | `8.6`, `8/X.4` | P29 |
| **T08** | Staff: the audit log | 2 | 2d | 11 | `9.3`, `9.3a`, `9.3b`, `9.3c`, `9.2d` | P30 |
| **T20** | Staff: contacts, POS ids, Telegram links, self-service | 2 | 2d | 9 | `9.2b`, `9.2c`, `9/X.1`, `9/X.5` | T08 |
| **T09** | Chart family and KPI tiles | 2 | 2d | 6 | `X.19`, `X.20` | P01 |
| **T21** | Rule authoring components | 2 | 2d | 4 | `X.25` | P01 |
| **T22** | Timeline, frames, rich text, steps, OTP, type stack | 2 | 2d | 11 | `X.26`, `X.28`, `X.31`, `X.33`, `X.38`, `X.40` | P01 |
...
| **W03** | Privacy self-service, access check and approvals | 3 | 2d | 8 | `10.11`, `9/X.4`, `9.4` | P30 |
| **W05** | Split tender: the payment[] array | 3 | 2d | 8 | `8/X.1` | P29 |
## The waves
