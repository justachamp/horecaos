...
|---|---|---|---|---|---|
| **P** — pilot | 114 | 32 | 4 | 1 | **151** |
| **2** — parity | 51 | 30 | 16 | 3 | **100** |
| **3** — tail | 13 | 7 | 8 | 2 | **30** |
| **?** — no IA row | 3 | 0 | 4 | 0 | **7** |
| **Total** | **181** | **69** | **32** | **6** | **288** |
...
40 rows — 28 built · 10 partial · 2 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `1.1` | Order board — queue shell, table and columns | P | PARTIAL | The board calls the cursor-paginated `GET .../orders/board` and renders three added columns — Оплата, Доставка (batch 8) and, this batch, Курьер: `OrderSummaryResponse.courierId` is now resolved through a new `fulfillment.api.ActiveCourierAssignmentsPort` (one batched read over `fulfillment.shipments` per page, not a board-query join) and rendered against the roster the toolbar's own filter already fetches — `courierId` used to be a board *filter* parameter only, never returned per row; now it is both, tested both ends (`order-queue.spec.ts`'s courier-column describe block). Customer is still not visible — `OrderSummaryResponse` carries only an opaque `customerAccountId`/hashed guest reference, never a display name — and branch is still absent because the board stays hard-scoped to one location via `CurrentLocation`. | L | — | w1-order-board-actions |  |
...
16 rows — 11 built · 3 partial · 1 not built · 1 blocked
...
| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `3.1` | Dispatch board | P | PARTIAL | The courier-keyed drag board (unchanged from the prior wave) gains, this batch, both of its two "still missing" console actions: bulk assignment (selecting several unassigned pool cards plus one courier issues one `DispatchApi.assign` call per plan, per-item applied/refused outcomes, since no bulk-assign endpoint exists) and 'call an external courier' (`«Вызвать курьера»` on the pool cards, see `1.2f`). Still missing: a map of points and routes (blocked on `X.4`), a non-PII destination label (zone/district + street without house number — `DeliveryOrderPort`'s `Waypoint` only carries one composed address string, not ordering's own structured fields; reported as `notDone`, needing new cross-module plumbing rather than a frontend change), and the 10-second poll still stands in for ADR 0045's SSE stream. | L | — | w4-dispatch-couriers |  |
...
25 rows — 14 built · 4 partial · 5 not built · 2 blocked
...
| `4.4d` | Catalog base settings (use stock logic; QR/kiosk take hall prices) | P | PARTIAL | `catalog.qr_kiosk_price_plane` is now actually enforced (wave 9 w7): the flag was already readable and writable at `/settings/catalog`, but `QuoteService`/`PriceQueryService` never consulted it, so setting it had zero effect on a real cart — `QrKioskHallPricing.resolve` now points a QR_TABLE/KIOSK channel with no price plane of its own at the tenant's single active POS channel when the switch is on, tested (`QuoteAndReservationTests`). `catalog.use_stock_logic` was re-verified against `InventoryService` and is already correctly enforced where it applies — but stays read-only in the console, since QUANTITY tracking itself remains unimplemented and no ADR decides what "turn stock logic on" should even do while it is: a merchant still cannot actually turn stock logic on for the company, reported as `notDone` rather than built around a decision no one has made. | M | — | w7-catalog-channel-controls |  |
| `4.5a †` | Catalog sync from POS — mapping table and run history | P | BUILT | — | L | — | P24 | NOT BUILT  |
| `4.5b †` | Excel import/export — templates, dry run with row-level results, image-by-URL fetch | P | PARTIAL | The backend now genuinely produces and consumes `.xlsx` (`org.dhatim:fastexcel`/`fastexcel-reader`, no Apache POI on the runtime path, bounded against a decompression-amplification upload by a fix round): `CatalogImportParser#parse` dispatches on the file name's extension, CSV and `.xlsx` rows resolve through the same normalize/toRow path, and `GET .../imports/template.xlsx` produces a real three-sheet workbook (Import/Examples/Reference), tested (`CatalogImportParserTests`). But the console still has no way to reach any of it: `q-import-wizard`'s `accept` input stays the default `'.csv'` (`catalog-import-page.html` passes none), the file is still read with `FileReader.readAsText` (which would corrupt a binary `.xlsx` selection), and the only template/export download offered is `downloadCsvText` — a "built, no consumer" gap by this document's own standard. No in-cell dropdowns/data validation either way; the closed vocabularies live on the workbook's own Reference sheet as text. | XL | — | w4-catalog-import |  |
...
| `6.4` | Campaigns — lifecycle, audience targeting, suppression | 2 | PARTIAL | `isWired`-gating, the scheduler-driven `APPROVED → SCHEDULED → SENDING` path, `POST /suppressions` and `getAudience`/`redefineAudience` (unchanged). This batch surfaces the two things the operator console still could not show: `CampaignDetailPane` now renders a halted banner and a "re-schedule" button exactly when a campaign is `SCHEDULED` with `scheduledAt` cleared and `haltedReason` set (the shape `POST .../reschedules` re-arms), and a "History & statistics" section fed by `recipientCounts` — queued/pending/deferred/refused/total, and the refused total's own breakdown by reason — honestly declining to fabricate a delivered/failed count, since `NotificationStatus` has no read receipt. `CampaignsPage`'s list gets a matching row-level hint. Only the audience-snapshot CSV export UI remains unbuilt. | M | — | w5-reports-exports |  |
...
39 rows — 28 built · 6 partial · 5 not built
...
| `7.2e` | Order reports — Excel/CSV export as an audited PII egress (plus the export centre) | P | PARTIAL | The audited chain (unchanged) is joined by two new registrations: `ReportExportRegistry` now recognizes `ORDER_REPORT_LOG` (the order-grain commercial log, one page bounded to the row quota) and `ORDER_REPORT_SUMMARY` (the branch/channel/fulfilment rollup, folded across date and entity axes) alongside `CUSTOMER_DIRECTORY` and wave 9's `ORDER_CRM_LOG` — all four wired end to end and tested (`ReportExportServiceTests`), and deliberately carrying no PII column, since combining `reporting.*` with ordering's CRM data server-side would re-link a fact to a customer, exactly what `SubjectPseudonym` (`7.2a`) exists to prevent. But `ExportCentrePage`'s own picker still offers only `CUSTOMER_DIRECTORY` as a selectable option — the screen's own doc comment says so outright — so an operator cannot actually trigger an order-log or order-summary export from `/statistics/exports`; only a direct `ReportingApi.requestExport({reportKey: ...})` call reaches the other three. Held at PARTIAL for the same "built, no consumer" reason this document applies consistently. | XL | — | w5-reports-exports |  |
...
20 rows — 8 built · 8 partial · 4 not built
...
| `9.2d †` | People — created-by / accepted-by attribution | 2 | PARTIAL | The order-detail half is now built: `OrderDetailResponse` resolves `createdByActorId`/`acceptedByActorId` to a display name through the existing `StaffDisplayNames` port (the same cached lookup `AuditQueryService` already uses for audit rows), and `order-detail-pane.ts`'s `actorDisplay` prefers the resolved name over the raw actor id, falling back to the old rendering only for a non-USER actor or a subject with no name on file. Tested both ends: `OperationsOrderControllerCourierAndAttributionHttpTests`, `order-detail-pane.spec.ts`. A manager still cannot see how many orders an operator took today from the person's own card — no staff-card aggregate exists yet. | M | — | w1-order-board-actions | PARTIAL |
