...
| **2** — parity | 60 | 23 | 14 | 3 | **100** |
| **3** — tail | 13 | 7 | 8 | 2 | **30** |
| **?** — no IA row | 3 | 0 | 4 | 0 | **7** |
| **Total** | **194** | **58** | **30** | **6** | **288** |
...
12 rows — 7 built · 2 partial · 3 not built
...
| `1.1c` | Filters (period, branch, aggregator, source, delivery type, courier, payment type) persisted per tab | P | PARTIAL | Eight of the named filters are real now — period, channel, fulfilment type, courier, payment method, «мои заказы», Источник (wave 9 w1) and, this batch, payment status (`paymentStatus`, reading `ordering.orders.payment_status_projection` directly, validated against its own seven values, distinct from the pre-existing payment-*method* filter) — all toolbar controls bound to `GET .../orders/board`'s query parameters, persisted per tab in `localStorage` (`order-queue-filter-state.ts`). The whole toolbar now also round-trips through the URL (`filtersToQueryParams`/`filtersFromQueryParams`/`hasFilterQueryParams`): a filter change pushes into the URL via `replaceUrl`, a pasted link or the browser's own back/forward becomes that load's source of truth, and switching tabs clears the previous tab's URL filters and hands control back to that tab's own `localStorage` memory. Still missing: a branch filter (the endpoint is already single-location-scoped; `CurrentLocation`'s shell picker switches branches instead), a filter on one specific aggregator binding rather than the coarser origin toggle (no read model exists for that yet), and the four booleans `orders.md` §2.4 itself marks not read by ordering. | L | — | w1-order-board-actions |  |
...
| `1.1e †` | Row action menu (inline + overflow, server-driven) | P | PARTIAL | Seven of `OrderActionCode`'s 9 values are now emitted: `AMEND_EMISSION_ENABLED` is `true` (wave P10) — the row's «Изменить» opens the order detail's own five-dialog amendment menu — `COMPLETE` (batch 8 w1) and, this batch, `ASSIGN_COURIER`: offered for a DELIVERY order between CONFIRMED and FULFILLING whose plan has nobody carrying it yet (`OrderActionsPolicy`'s new 4-argument overload, gated on the existing `Capability.DELIVERY_MANUAL_ASSIGN`), the row action opening the order and reusing the assign/unassign control the detail pane already has rather than a second picker. Tested both ends (`OrderActionsPolicyTests`, `OperationsOrderControllerCourierAndAttributionHttpTests`, `order-queue.spec.ts`). `RESOLVE` and `ISSUE_INVOICE` remain declared but never emitted — each still waits on its own endpoint (amendment confirmation, a re-issued invoice). A terminal order still gets only the minimal read-only overflow (Открыть/Копировать номер). | S | — | w1-order-board-actions | BUILT |
...
| `1.2c †` | Amendments — add items, edit customer/address/type/pre-order time, change payment type | P | PARTIAL | Six of the seven financial commands now carry `built()=true` on `AmendmentCommandType` and have a real console dialog each — `ADD_LINES` (increases only, reusing the New Order composer's search), `CHANGE_LINE_QUANTITY` (increase only — a decrease still has no ADR 0017 return/write-off primitive), `CHANGE_PAYMENT_METHOD` (`CASH` at either end, or opening a fresh online intent), `CHANGE_DELIVERY_ADDRESS` (repriced through `CartService#price`, refused out of zone), `CHANGE_FULFILLMENT_TIME` and `CHANGE_CONTACT` — all wired into `order-detail-pane.ts`'s amend menu beside the five pre-existing non-financial commands, each proved through `OrderAmendmentAndOutcomeTests` and a new `OperationsOrderControllerAmendmentsHttpTests` (the amend endpoint's first HTTP-level test class, catching a real defect: `AmendmentRefusedException` fell through to a bare 500 instead of the 409 every sibling refusal returns, now fixed). Only `REMOVE_LINES` stays refused by name — releasing or writing off already-committed stock needs a fourth ADR 0017 primitive (return-to-stock or write-off) beyond the port's deliberate three (hold, commit, release), a decision this wave does not make. Held at PARTIAL for that one remaining command. | XL | Removing/decreasing a line needs an ADR 0017 primitive (return-to-stock or write-off) the port does not have; that module's decision, not this wave's. | w2-financial-amendments | PARTIAL |
...
16 rows — 12 built · 2 partial · 1 not built · 1 blocked

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `2.1 †` | Kitchen queue (KDS) — the live board | P | BUILT | A cook can reveal a line's note on demand, the KDS has a real `aggregator` tab, and tab/board counts are exact server-side (unchanged from batch 10). This batch closes all three of the row's remaining gaps: `KitchenTicketService` now publishes an ADR 0045 `KITCHEN_BOARD` realtime signal on every ticket/item mutation, the desk-console `kitchen-queue-page.ts` subscribes to it through the shared `RealtimeClient` (a signal or a resync now shortens the 10s poll rather than the poll standing in for the stream), and a new `/wallboard/kitchen` touch shell — hosted outside the console Shell like the existing wallboard — gives a tablet or wall display every finger-sized action with no hover-only affordance and its own offline/disconnected banner that names how long the board has actually been stale, independent of transport state. Tested both ends (`KitchenExecutionTests`'s `KITCHEN_BOARD`-publish cases, `kitchen-queue-page.spec.ts`, `wallboard-kitchen-page.spec.ts`). | M | — | w4-kitchen-realtime |   |
...
| `2.1b †` | Preset product comments on a kitchen line | P | BUILT | The whole round trip is now real, closing every gap the wave's own commits used to name: an operator (New Order's item-modifier dialog) or a customer (the storefront product screen) can attach a coded preset comment to a line, `CartService.putLine`'s new `commentPresetCodes` argument validates each against `CommentPresetLookup#offeredCodesForVariant`, `order_line_comment_presets` snapshots the label at checkout, the KDS ticket pass renders each line's presets, and `PosOrderExportService` maps them to POS modifier codes through a new `COMMENT_PRESET` entity type (unmapped refuses `MODIFIER_UNMAPPED`, and a fix round closed a gap where `findUnmappedEntity` did not check preset mappings at all). Tested both ends across all three surfaces: `CartCheckoutAndOrderTests`, `OrderPosExportControllerTests`, `kitchen-queue-page.spec.ts`, `item-modifier-dialog.spec.ts`, `product.component.spec.ts` (storefront). | L | — | w3-catalog-presets-schedule-xlsx |  |
...
| `2.1d †` | Change payment type from the KDS | P | BUILT | `CHANGE_PAYMENT_METHOD` now carries `built()=true` (wave 10 row `1.2c`) and the KDS ticket pass gets its own «Изменить оплату» action reusing the same payment-method picker the order-detail dialog renders, so a KDS operator and a console operator choose from an identical list. No pre-check: a refusal (`PAYMENT_METHOD_CHANGE_REQUIRES_VOID_REFUND` for a provider-paid order moving to another online method) surfaces as an `actionNotice` band after the attempt rather than a proactively greyed-out option — a documented simplification, not a gap. Tested: `kitchen-queue-page.spec.ts` (231 new lines). | S | — | w2-financial-amendments |  |
...
15 rows — 5 built · 8 partial · 2 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `3.1` | Dispatch board | P | PARTIAL | The courier-keyed drag board, bulk assignment and 'call an external courier' are unchanged from batch 10. This batch closes the non-PII destination label gap: ordering now computes a masked `DeliveryDestination` projection (district/zone and street, never a house number, a flat or a phone, mirroring `PhoneMasking`'s own safely-biased-truncation precedent), snapshotted onto `fulfillment.delivery_plans` at plan creation (V0410) so the board's 10-second poll never decrypts anything; `DispatchController.PlanQueueResponse` carries it and the queue card renders it, tested both ends (`DeliverySourcingTests`, `dispatch-board-page.spec.ts`). Still missing: a map of points and routes (blocked on `X.4`), and the 10-second poll still stands in for ADR 0045's SSE stream — the dispatch board itself was not wired to `RealtimeClient` this batch, unlike the kitchen board (row 2.1). | L | — | w5-fulfillment-destination |   |
...
| `4.8a` | Price books — the named price planes and their assignment | 2 | PARTIAL | Price-book assignment offers BRAND/LOCATION/CHANNEL scope with priority/validFrom/validUntil (unchanged), and VAT/tax profiles are no longer write-only: `PriceAuthoringController` gains `GET /tax-profiles` (every jurisdiction the brand has a rate in force for) and `GET /tax-profiles/{jurisdictionCode}`, and the console's tax-profile screen now loads the brand's existing profiles and shows what is currently in force before the edit form is touched. Tested both ends: `PriceAuthoringControllerEndpointTests`, `tax-profile-page.spec.ts`. A book's variant prices can still only be seen one product at a time in the editor. | M | — | w6-settings-locations-policy |  |
...
15 rows — 4 built · 4 partial · 6 not built · 1 blocked
...
| `6.5 †` | Automations — birthday, cashback change, late-order apology, inactivity/abandonment triggers | 2 | PARTIAL | Three of the five named triggers now fire without a human: V0414/V0415 add `marketing.automation_rules`/`automation_runs`, authored inert and armed only by a separate, `campaign.approve`-gated activation (`ck_automation_rule_human_armed` plus the service both enforce "nothing sends without a human"). `AutomationFiringService` claims a per-(rule, customer, guard-key) row, applies the same five `MarketingEligibility` subtractions and quiet-hours deferral a campaign recipient gets, then reuses `CampaignMessagePort` — one send path, not a second one. BIRTHDAY guards on a calendar year in the brand's timezone; INACTIVITY and CART_ABANDONMENT guard on a fixed-width day bucket; CART_ABANDONMENT also checks `OrderDirectory` inside the same transaction as its guard claim (closing a TOCTOU a fix round caught) so a cart that converts first is recorded CANCELLED, not sent. A new `AutomationTriggerScheduler` sweep covers all three hourly. Tested (`AutomationTests`, `AutomationRuleControllerEndpointTests`). CASHBACK_CHANGE and LATE_ORDER_APOLOGY are still not offered as trigger kinds: no accrual/debit event exists in `loyalty` today for the first (the row's own note claiming one is emitted was stale), and ADR 0044 states the late-order apology is "deliberately absent" pending the still-Proposed ADR 0112 for the second — neither is a decision this batch can make on its own. | XL | — | w8-marketing-automations |   |
...
39 rows — 32 built · 2 partial · 5 not built
...
| `8.6` | Subscription & billing | 2 | PARTIAL | Plan, entitlements, usage, statements, the purchasable-module catalogue with inline purchase, the tenant-visible arrears state and restricted-feature banner are all built (batch 10/ADR 0088). This batch closes ADR 0127's own named gap: clicking "Add" now opens `q-confirm-dialog` naming the module, its total price (quantity folded in for a PER_UNIT module), its billing unit and what it switches on, before `CommercialApi.purchaseModule` is ever called — Cancel/Escape/backdrop send nothing. Still absent: a tenant self-service 'end' to undo a purchase (`CommercialModuleController.end` stays PLATFORM-scope) and the prepaid wallet (`8/X.3`, still blocked on ADR 0095). | L | — | w1-quick-closes |   |
...
20 rows — 10 built · 6 partial · 4 not built
...
| `9.3a` | Activity & audit — field-level before/after diff | 2 | PARTIAL | `ChangeDocuments.diff` is unchanged. Migration coverage grows from 2 of roughly 130 `.changed(...)` call sites to 10: `tenant.suspended`/`.reactivated`/`.activated` (via `TenantControlPlaneService`'s shared helper), `TenantProfileService` (business type, country), `LegalEntityService.update` (now also covering `registeredAddress`/`contactPhone`, redacted but at least shown as changed — the old flat map omitted them outright under ADR 0029), `BranchTagService`'s tag-assignment set, `CatalogAuthoringService.setProductStatus` and `MenuAuthoringService.updateMenu`, and `CourierTypeService.updateType`. Each site's diff has a test asserting the new `{before, after}` shape. Roughly 120 of ~130 sites still write a flat, after-only map — no order-policy call site is included, since `OrderAcceptancePolicyService`/`OrderLatenessPolicyService`/`OrderOutcomeReasonService` currently write no audit fact at all, a different, larger gap than this row's own migration. | L | — | w7-audit-people |   |
...
36 rows — 27 built · 8 partial · 1 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `10.0` | Settings home & "Find a setting" | P | PARTIAL | The readiness panel now names every offending item for all four VALIDATING-phase steps (wave 9 w3, unchanged) plus, this batch, a fifth check — `OnboardingService.validate()` also dry-runs the tenant's SMS/notification-template readiness. Find-a-setting is no longer only a label/description filter: the same input now also drives a real `q-combobox` dropdown over `ConfigurationKeys.all()` (ADR 0030, via a new `ConfigurationApi.keys`) and `reference-data-page.ts`'s five named lists, each individually searchable and navigable — a key with no console consumer is not offered at all rather than offered with nowhere to go. Still narrower than settings.md's ambition: no fiscal assignment, channel-payment coverage, or secret-rotation-age checks exist anywhere. Tested: `settings-home-page.spec.ts`, `OnboardingServiceTests`. | M | — | w6-settings-locations-policy |  |
...
| `X.25` | ConditionBuilder + RuleList with priority reorder + RuleSimulator | 2 | PARTIAL | `q-condition-builder` (wired into `segments-page`) is unchanged. This batch gives `q-rule-list` its first live consumer: the new `/marketing/automations` console page (row 6.5) renders the priority-ordered automation-rule list through it, with drag/keyboard reorder persisted through one whole-set `PUT .../automations/reorder` call and its enabled toggle wired to separate activate/deactivate endpoints. `q-rule-simulator` (candidate-driven match preview) still has no live consumer — the promotion and dispatch-rule screens that would use it remain future waves. | L | — | w8-marketing-automations |   |
