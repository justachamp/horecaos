...
|---|---|---|---|---|---|
| **P** — pilot | 96 | 47 | 7 | 1 | **151** |
| **2** — parity | 48 | 32 | 17 | 3 | **100** |
| **3** — tail | 12 | 8 | 8 | 2 | **30** |
| **?** — no IA row | 3 | 0 | 4 | 0 | **7** |
| **Total** | **159** | **87** | **36** | **6** | **288** |
...
40 rows — 22 built · 15 partial · 3 not built
...
| `1.1c` | Filters (period, branch, aggregator, source, delivery type, courier, payment type) persisted per tab | P | PARTIAL | Seven of the named filters are real now — period, channel, fulfilment type, courier, payment method, «мои заказы» and, since wave 9 w1, Источник (origin: HORECAOS vs MARKETPLACE, `ordering.orders.origin`, V0038) — all toolbar controls bound to `GET .../orders/board`'s query parameters, persisted per tab in `localStorage` (`order-queue-filter-state.ts`), tested for navigation/reload/tab-independence. Still missing: a branch filter (the endpoint is already single-location-scoped; `CurrentLocation`'s shell picker switches branches instead), a filter on one specific aggregator binding rather than the coarser origin toggle (no read model exists for that yet), a payment-status filter, the four booleans `orders.md` §2.4 itself marks not read by ordering, and URL round-tripping — a filtered board is not yet a shareable link. | L | — | w1-queue-override-filters |  |
...
16 rows — 9 built · 4 partial · 2 not built · 1 blocked
...
25 rows — 13 built · 5 partial · 5 not built · 2 blocked
...
39 rows — 25 built · 9 partial · 5 not built
...
| `7.2a` | Order reports — commercial/CRM order log («Заказы») | P | PARTIAL | The CRM half is now built (wave 9 w4): `GET /orders/crm-log` (`OrderCrmLogController`, `ORDER_READ` at TENANT) answers customer/operator/courier entirely from ordering/fulfillment, never touching `reporting.*` — `SubjectPseudonym` exists specifically so a reporting fact is never re-linked to a customer, proved by a source-scan test alongside the behavioural ones. «Заказы» renders Клиент/Оператор/Курьер joined client-side by orderId (name in full, phone masked, courier by its non-PII `display_reference`), and the export centre gained `ORDER_CRM_LOG` as a registered export. Still no column chooser or saved views on this screen — `order-reports-page.ts`'s own comment notes both as out of scope. Tested both ends: `OrderCrmLogTests`, `order-reports-page.spec.ts`'s CRM-column cases. | L | — | w4-reports-distance-crm |  |
...
36 rows — 21 built · 13 partial · 2 not built

| # | Row | Tier | Status | What is missing | Size | Blocked by | Wave | Reader said |
|---|---|---|---|---|---|---|---|---|
| `10.0` | Settings home & "Find a setting" | P | PARTIAL | The readiness panel now names every offending item for all four VALIDATING-phase steps, not only payment (wave 9 w3): `DeliveryConfigurationValidate`, `PosBindingsValidate` and `CatalogReadinessValidate` were brought to `PaymentConfigurationValidate`'s own `StepResult.failedWithFindings` shape, over the same generic frontend rendering path — no cross-cutting plumbing changed, tested per-handler in `OnboardingStepHandlersTests`. Still narrower than settings.md's ambition: the panel only checks what `OnboardingController.validate` already covers (no fiscal assignment, channel-payment coverage, notification moderation or secret-rotation-age checks exist anywhere), and find-a-setting is still a label/description filter over the nav index, not the full Combobox over every configuration key and reference list the spec asks for — untouched by this batch. | M | — | w3-branches-settings-readiness |  |
...
| `10.1` | Brand profile | P | PARTIAL | Logo and banner now go through the real `q-media-uploader` (wave 9 w2, 1:1 crop for the logo, 3:1 for the banner) instead of an operator pasting a raw asset UUID — the same request/PUT-bytes/finalize flow product photos already use, targeted at BRAND; both the read view and the edit form show the current image as a thumbnail (`MediaAssetService.downloadUrl` `THUMBNAIL`). Rename, contact phone, Telegram handle and the supported-locale set are otherwise unchanged from before this batch. Still no country/currency/timezone display. Tested: `brand-profile-page.spec.ts` gains the upload/thumbnail cases. | L | — | w2-brand-channel-presentation |  |
...
| `10.8a` | Integrations — hub: provider installs, connect fields, secret door, rotation, bindings | P | PARTIAL | A per-branch install model now exists (wave 9 w8) — `GET .../branches/effective-bindings` shows, per branch, which installation resolves each capability and whether it is a location override or the brand default, and a "Bind to a branch" dialog lets the console bind an existing installation to one. This is narrower than the wave's own report suggests, though: a fix-round bug found the bind dialog offered POS/DELIVERY installations with an empty `capabilities: []`, which `integration.bindings`'s INNER JOIN to `integration.binding_capabilities` can never resolve — a dead binding that looked like it worked. The fix restricts the picker to categories with no capability catalogue (matching the connect drawer's own `onBind`), so PAYMENT/NOTIFICATION installations can be bound to a branch from the console today, but POS and DELIVERY — the categories this row's own capability-reconciliation read most centrally concerns — still cannot be, pending a capability-assignment picker this dialog does not have; the read side resolves every category per branch, only the write side is category-limited. Retiring an installation stays out of scope per ADR 0065, unchanged. Tested: `ProviderInstallationControllerTests`, four new `integrations-page.spec.ts` cases including the POS/DELIVERY exclusion. | M | — | w8-notification-templates-installs |  |
...
| `X.12` | MediaUploader with aspect-ratio crop (+ video) | P | PARTIAL | Logo/banner now go through `q-media-uploader` too (wave 9 w2, see `10.1`), closing the one gap this row's own note used to name. Still no upload path anywhere for kiosk idle media, story slide or review-tag icon, and video is still refused server-side — the uploader passes a video file straight through and `MediaAssetService`'s image-only pipeline rejects it, unchanged by this batch. | M | — | w2-brand-channel-presentation |  |
