...
| 1.5 | Reservations | Table bookings: create, edit, cancel, complete. **Owns:** Бронирования list and detail; multi-table bookings; ~~auto-create client on unknown phone~~; ~~external reservation ID as the displayed identifier~~ *(both struck 2026-09-11: [ADR 0047](adr/partial/0047-dine-in-table-service-and-qr-ordering.md) decides that a booking for a guest with no account creates no customer record and no consent, and that the external reservation id lives once in `integration.provider_entity_mappings` as a lookup key rather than as the identifier the screen displays)*; time-interval availability per table. (Floor plan is configured in 10.2.) | 3 |
| 1.6 | Call centre | Inbound/outbound telephony: screen-pop with caller lookup and call log. **Owns:** ~~softphone~~ *(struck 2026-09-11: [ADR 0064](adr/partial/0064-voice-channels-and-the-operator-presence-model.md)'s Alternatives refused a softphone/WebRTC inside operations — «carries audio, codecs, and telephony reliability into this platform's scope for little pilot value; operators keep their handsets»; click-to-call goes with it, since dialling needs the same client)*; call list; caller→customer resolution; the raw inputs to operator KPIs. | 3 |
| 1.7 | Inbox (conversations) | **Exists today** — added 2026-09-11 to record a screen the IA never listed; shipped under [ADR 0059](adr/partial/0059-conversational-engagement-and-the-sendpulse-exit.md) stage 2 as operations `/inbox`. Customer conversations from the storefront bot, needs-attention first. **Owns:** conversation list (no message bodies in the list payload) and audited history read; reply; take over from the flow and return to it; close, and reopen on an inbound message. Named gaps: no needs-attention count badge on the rail, and Telegram is the only channel adapter. | P |
...
| 2.7 | Kitchen device enrolment | **Exists on the backend** — added 2026-09-11 because 10.5 registers kiosks, not kitchen devices, and the IA had no row for this at all. [ADR 0079](adr/partial/0079-kitchen-display-device-principal-and-enrolment.md)'s approve / list / revoke endpoints are complete and have no console caller yet. **Owns:** the per-branch KDS device registry; approving the enrolment code a screen displays; revoking a lost or replaced tablet. | P |
...
| 3.6 | Delivery zones | Draw the polygons that decide serviceability, branch selection, tariff and provider. **Owns:** зоны доставки; branch geozones; regions with SW/NE bounding box constraining the geocoder; ~~free geozone~~ a DELIVERY zone whose tariff resolves to zero, shown as free *(reworded 2026-09-11: `fulfillment/domain/zone/ZoneRole.java` — «a 'free geozone' is not a third role: it is a DELIVERY zone whose tariff resolves to zero, and expressing it as a layer is what left three layers with no documented interaction»; [ADR 0037](adr/partial/0037-delivery-zones-tariffs-and-fee-resolution.md) refuses a third geometry layer)*; zone → branch set; zone → courier-service binding; zone-beats-branch tariff precedence; bulk geozone upload. *(HorecaOS should collapse Delever's three overlapping geometry layers into one Zone entity with typed roles.)* | P |
...
| 6.7 | Content | Everything customer-facing that is not a product. **Owns:** banners (image **or video**, priority, per-channel placement, active period); stories (group → ordered slides with per-slide media, duration, CTA, view counts); pop-ups (delay, duration, priority, time window, **plus frequency capping** — a HorecaOS addition); promotion content cards **linked to the promotion rule** (HorecaOS improvement: Delever keeps them unrelated so ads drift out of sync with pricing). ~~News; gallery albums; recipes (editorial); vacancies (a public listing only)~~ *(struck 2026-09-11: the parity matrix excludes a general-purpose editorial CMS with tenant-authored raw HTML, and [ADR 0044](adr/partial/0044-marketing-campaigns-audiences-and-engagement.md) repeats it — the platform builds no editorial CMS, only four bounded merchandising placements with no tenant-authored markup; ADR 0044 names static pages, news, galleries, recipes, tenders and job postings as not built)*. | 2 |
...
| 7.7 | Product analytics | **Owns:** ABC by revenue share with cumulative; XYZ by stdev / coefficient of variation; the ABC×XYZ matrix as filters; product report; ~~kiosk sales report~~ kiosk readable as a channel slice of the sales report *(replaced 2026-09-11: [statistics.md](operations-spec/statistics.md) §2.7 declines a separate kiosk report — «a separate report for one channel is a precedent that ends in eleven reports»)*. | 2 |
...
- **A recruitment ATS, and job postings at all.** ~~Vacancies stay as content (6.7)~~ *(struck 2026-09-11: the parity matrix excludes the general-purpose editorial CMS and [ADR 0044](adr/partial/0044-marketing-campaigns-audiences-and-engagement.md) names job postings among what is not built, so vacancies are not a content surface either)*; a candidate pipeline collecting free-text CVs and phone numbers from anonymous public users is a retention liability with no revenue attached.
...
| **RichTextEditor** (block-based, sanitized) | 10.5 static pages (Delever accepts **raw HTML from tenants** — an XSS surface HorecaOS must close). ~~6.7 news and vacancy bodies~~ *(struck 2026-09-11: the parity matrix excludes the general-purpose editorial CMS and [ADR 0044](adr/partial/0044-marketing-campaigns-audiences-and-engagement.md) repeats it, so no editorial body copy is authored here.)* |
