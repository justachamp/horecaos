...
# Installation
# ------------
#   */1 * * * * /opt/qoida/qoida-platform/infra/observability/qoida-probe.sh
#
...
#
# Configuration is /etc/qoida/alerting.env, root-owned, mode 0600. The URLs in it
...
STATE_DIR="${QOIDA_PROBE_STATE_DIR:-/var/lib/qoida/probe}"
RUNBOOKS="${QOIDA_RUNBOOK_BASE:-/opt/qoida/qoida-platform/docs/runbooks}"
...
STALL_SECONDS="${QOIDA_STALL_SECONDS:-900}"
CONSUMER_LAG_THRESHOLD="${QOIDA_CONSUMER_LAG_THRESHOLD:-500}"
CALLBACK_FAILURES="${QOIDA_CALLBACK_FAILURES:-3}"
CALLBACK_WINDOW_SECONDS="${QOIDA_CALLBACK_WINDOW_SECONDS:-300}"
CIRCUIT_OPEN_SECONDS="${QOIDA_CIRCUIT_OPEN_SECONDS:-600}"
FENCE_BURST="${QOIDA_FENCE_BURST:-10}"
FENCE_WINDOW_SECONDS="${QOIDA_FENCE_WINDOW_SECONDS:-300}"
BACKUP_MAX_AGE_HOURS="${QOIDA_BACKUP_MAX_AGE_HOURS:-26}"
DISK_THRESHOLD_PERCENT="${QOIDA_DISK_THRESHOLD_PERCENT:-85}"
CERTIFICATE_MIN_DAYS="${QOIDA_CERTIFICATE_MIN_DAYS:-7}"
ONBOARDING_STALL_SECONDS="${QOIDA_ONBOARDING_STALL_SECONDS:-3600}"
...
TRADING_START="${QOIDA_TRADING_START:-0900}"
TRADING_END="${QOIDA_TRADING_END:-2330}"
...
NIGHT_REPEAT_SECONDS="${QOIDA_NIGHT_REPEAT_SECONDS:-1800}"
TRADING_REPEAT_SECONDS="${QOIDA_TRADING_REPEAT_SECONDS:-3600}"

# shellcheck disable=SC1090
[ -r "${CONFIG}" ] && . "${CONFIG}"

export TZ="${QOIDA_TIMEZONE:-Asia/Tashkent}"
...
clear_alert() {
    ...
    if [ "${tier}" = "night" ]; then
        push "${QOIDA_ALERT_WEBHOOK_LOUD:-${QOIDA_ALERT_WEBHOOK:-}}" \
            "qoida RECOVERED: ${description}"
    fi
}
...
alert() {
    ...
    local text
    text="qoida $(printf '%s' "${tier}" | tr '[:lower:]' '[:upper:]'): ${description} -> ${RUNBOOKS}/${runbook}"

    case "${tier}" in
        night)
            if may_notify "${key}" "${NIGHT_REPEAT_SECONDS}"; then
                if ! push "${QOIDA_ALERT_WEBHOOK_LOUD:-${QOIDA_ALERT_WEBHOOK:-}}" "${text}"; then
        ...
        trading)
            if in_trading_hours; then
                if may_notify "${key}" "${TRADING_REPEAT_SECONDS}"; then
                    push "${QOIDA_ALERT_WEBHOOK_LOUD:-${QOIDA_ALERT_WEBHOOK:-}}" "${text}"
                fi
            else
...
send_digest_if_due() {
    ...
    local body
    body="qoida morning digest ${today}:"$'\n'"$(cut -f2- "${DIGEST}")"
    if push "${QOIDA_ALERT_WEBHOOK_QUIET:-${QOIDA_ALERT_WEBHOOK:-}}" "${body}"; then
        printf '%s\n' "${today}" > "${stamp}"
        : > "${DIGEST}"
    fi
}
...
check_database() {
    if compose exec -T platform-db pg_isready -U qoida_migrator -d qoida >/dev/null 2>&1; then
...
check_order_flow() {
    ...
    outbox="$(metric_sum qoida_outbox_oldest_pending_age_seconds)"
    inbox="$(metric_sum qoida_inbox_oldest_pending_age_seconds)"
    lag="$(consumer_lag)"
...
consumer_lag() {
    compose exec -T kafka /opt/kafka/bin/kafka-consumer-groups.sh \
            ...
            --group "${QOIDA_INBOX_GROUP_ID:-qoida-platform}" 2>/dev/null \
...
#    manual investigation queue, so this is a customer's money in a state Qoida
...
check_provider_circuits() {
    ...
    open_for="$(metric_max qoida_provider_circuit_open_duration_seconds 'family="(payment|pos)"')"
    if [ -z "${open_for}" ] || [ "${open_for}" -lt "${CIRCUIT_OPEN_SECONDS}" ]; then
        clear_alert circuit-open "the provider circuit has closed" trading
        return
    fi
    provider="$(metric_max_label qoida_provider_circuit_open_duration_seconds provider 'family="(payment|pos)"')"
...
check_monetary_dead_letters() {
    local total delta
    total="$(metric_sum 'qoida_(outbox|inbox)_dead_letters' 'monetary="true"')"
...
check_fence_burst() {
    ...
    total="$(metric_sum qoida_migration_writes_fenced_total)"
    [ -z "${total}" ] && return
    ...
    capability="$(metric_max_label qoida_migration_writes_fenced_total capability)"
...
check_disk() {
    ...
    used="$(df -P "${QOIDA_DATA_VOLUME:-/}" 2>/dev/null | awk 'NR == 2 { gsub("%", "", $5); print $5 }')"
    [ -z "${used}" ] && return
...
check_onboarding_stalled() {
    local age
    age="$(metric_max qoida_onboarding_runs_stalled_age_seconds)"
    [ -z "${age}" ] && return
...
if [ "${database_up}" = "no" ]; then
    ...
    defer_to_digest app-unreachable \
        "qoida: the application did not answer /actuator/prometheus from inside the network"
    firing=1
fi
