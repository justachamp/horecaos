package uz.qoida.platform.reporting.web;
...
import uz.qoida.platform.reporting.application.MetricSigningService;
import uz.qoida.platform.reporting.application.ReportingRefusals;
...
public class ReportingApiErrorHandler {
    ...
    ProblemDetail unknownMetric(MetricRegistry.UnknownMetricException exception) {
        return ApiProblem.withProperties(ErrorCode.VALIDATION_FAILED, exception.getMessage(),
    ...
    ProblemDetail combinedEntityTotal(ReportingRefusals.CombinedEntityTotalException exception) {
        return ApiProblem.withProperties(ErrorCode.VALIDATION_FAILED, exception.getMessage(),
                Map.of("reason", "LEGAL_ENTITY_GROUPING_REQUIRED",
                        "metricCodes", exception.metricCodes()));
    }
    ...
    ProblemDetail alreadySigned(MetricSigningService.AlreadySignedException exception) {
        return ApiProblem.withProperties(ErrorCode.RESOURCE_CONFLICT, exception.getMessage(),
                Map.of("reason", "METRIC_ALREADY_SIGNED"));
    }
}
