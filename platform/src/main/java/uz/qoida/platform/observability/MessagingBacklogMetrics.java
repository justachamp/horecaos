...
public class MessagingBacklogMetrics {
    ...
    public MessagingBacklogMetrics(JdbcClient jdbc, Clock clock, MeterRegistry meters) {
        ...
        this.outboxDeadLetters = MultiGauge.builder("qoida.outbox.dead.letters")
                .description("Outbox rows in DEAD_LETTER, by topic domain")
                .register(meters);
        this.inboxDeadLetters = MultiGauge.builder("qoida.inbox.dead.letters")
                .description("Inbox rows in DEAD_LETTER, by topic domain and failure category")
                .register(meters);
    }

    @Scheduled(
            initialDelayString = "${qoida.observability.metrics.initial-delay:PT10S}",
            fixedDelayString = "${qoida.observability.metrics.interval:PT15S}")
    public void refresh() {
        try {
            refreshOutbox();
     ...
     */
    @Scheduled(
            initialDelayString = "${qoida.observability.metrics.inbox-initial-delay:PT25S}",
            fixedDelayString = "${qoida.observability.metrics.inbox-interval:PT1M}")
