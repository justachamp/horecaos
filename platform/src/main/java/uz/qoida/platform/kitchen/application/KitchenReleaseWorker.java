 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.kitchen.release-worker.enabled", havingValue = "true",
        matchIfMissing = true)
public class KitchenReleaseWorker {
    ...
    public KitchenReleaseWorker(KitchenTicketService tickets,
            @Value("${qoida.kitchen.release-worker.batch-size:50}") int batchSize) {
        this.tickets = tickets;
        this.batchSize = batchSize;
    }

    @Scheduled(
            initialDelayString = "${qoida.kitchen.release-worker.initial-delay:PT10S}",
            fixedDelayString = "${qoida.kitchen.release-worker.interval:PT5S}")
