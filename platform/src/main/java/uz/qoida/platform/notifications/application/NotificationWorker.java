 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.notifications.worker.enabled", havingValue = "true",
        matchIfMissing = true)
public class NotificationWorker {
    ...
    public NotificationWorker(JdbcNotificationStore notifications,
            ...
            Clock clock,
            @Value("${qoida.notifications.worker.batch-size:50}") int batchSize,
            @Value("${qoida.notifications.worker.lease:PT2M}") Duration lease) {
    ...
    @Scheduled(
            initialDelayString = "${qoida.notifications.worker.initial-delay:PT10S}",
            fixedDelayString = "${qoida.notifications.worker.interval:PT5S}")
    public void runOnce() {
        try {
            drain();
