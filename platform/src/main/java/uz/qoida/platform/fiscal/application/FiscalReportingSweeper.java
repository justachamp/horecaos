 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.fiscal.reporting-sweeper.enabled", havingValue = "true",
        matchIfMissing = true)
public class FiscalReportingSweeper {
    ...
    public FiscalReportingSweeper(FiscalDocumentService documents,
            @Value("${qoida.fiscal.reporting-sweeper.batch-size:200}") int batchSize) {
     ...
     */
    @Scheduled(
            initialDelayString = "${qoida.fiscal.reporting-sweeper.initial-delay:PT30S}",
            fixedDelayString = "${qoida.fiscal.reporting-sweeper.interval:PT1M}")
