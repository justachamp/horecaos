 ...
 *                 else's primary
 */
@ConfigurationProperties(prefix = "qoida.migration.legacy")
public record LegacySourceProperties(
    ...
    public LegacySourceProperties {
        ...
        if (enabled) {
            requirePresent(url, "qoida.migration.legacy.url");
            requirePresent(username, "qoida.migration.legacy.username");
            requirePresent(passwordReference, "qoida.migration.legacy.password-reference");
