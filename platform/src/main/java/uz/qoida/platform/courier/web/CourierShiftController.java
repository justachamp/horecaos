...
import uz.qoida.platform.courier.api.CourierSelfAuthorized;
...
public class CourierShiftController {
    ...
    public CourierShiftController(CourierShiftService shifts, CourierCashService cash,
            JdbcCourierStore couriers, CurrentActor currentActor) {
        this.shifts = shifts;
        this.cash = cash;
    ...
    @Idempotent
    @Operation(summary = "Open my shift",
    ...
    public ResponseEntity<ShiftResponse> open(
            ...
            @PathVariable UUID locationId, @Valid @RequestBody OpenShiftRequest body) {

        UUID courierId = me(tenantId);
        ShiftRow shift = shifts.open(new CourierShiftService.OpenShift(tenantId, brandId, locationId,
                courierId, ShiftActor.COURIER, actor(), "Courier opened their own shift",
                body.point(), body.currency()));
    ...
    @CourierSelfAuthorized(Capability.COURIER_SHIFT_OPEN)
    @Idempotent
    @Operation(summary = "Close my shift",
            description = "Ends any open break first, so its seconds are not paid.")
    public ResponseEntity<CloseResponse> close(
            @PathVariable UUID tenantId, @PathVariable UUID brandId,
            @PathVariable UUID locationId, @PathVariable UUID shiftId,
        ...
        CourierShiftService.CloseOutcome outcome = shifts.close(new CourierShiftService.CloseShift(
                tenantId, shiftId, ShiftActor.COURIER, actor(), null,
                "Courier closed their own shift", body.point(), body.currency()));

        return ResponseEntity.ok(new CloseResponse(outcome.status().name(), outcome.paidSeconds(),
                outcome.breakSeconds(), outcome.cashHandoverId()));
    }

    @PostMapping("/shifts/{shiftId}/breaks")
    @CourierSelfAuthorized(Capability.COURIER_SHIFT_BREAK)
    @Idempotent
    @Operation(summary = "Start my break",
    ...
    public ResponseEntity<Void> startBreak(
        ...
        requireOwnShift(tenantId, shiftId);
        shifts.startBreak(tenantId, shiftId, ShiftActor.COURIER, actor(),
                "Courier started their own break");
    ...
    public ResponseEntity<Void> endBreak(
        ...
        requireOwnShift(tenantId, shiftId);
        shifts.endBreak(tenantId, shiftId, ShiftActor.COURIER, actor(),
                "Courier ended their own break");
        return ResponseEntity.accepted().build();
    }
    ...
    @CourierSelfAuthorized(Capability.COURIER_SHIFT_OPEN)
    @Idempotent
    @Operation(summary = "Declare the cash I am handing over",
    ...
    public ResponseEntity<Void> declareCash(
            @PathVariable UUID tenantId, @PathVariable UUID brandId,
            @PathVariable UUID locationId, @PathVariable UUID handoverId,
    ...
    private UUID me(UUID tenantId) {
        return couriers.findCourierBySubject(tenantId, currentActor.get().subject())
                ...
                .orElseThrow(() -> new ApiException(ErrorCode.RESOURCE_NOT_FOUND,
                        "The caller is not a courier of this tenant"));
    }
    ...
    record OpenShiftRequest(@Size(max = 64) String point, @Size(min = 3, max = 3) String currency) { }

    record CloseShiftRequest(@Size(max = 64) String point, @Size(min = 3, max = 3) String currency) { }

    record CashDeclarationRequest(long declaredMinor) { }

    record ShiftResponse(UUID shiftId, String status, String dutyState, String enforcementMode,
            int version) {

        static ShiftResponse of(ShiftRow shift) {
            return new ShiftResponse(shift.id(), shift.status().name(), shift.dutyState().name(),
                    shift.enforcementMode().name(), shift.version());
        }
    }
