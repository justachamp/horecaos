...
import uz.qoida.platform.courier.domain.CourierAccrual;
import uz.qoida.platform.courier.domain.CourierCompensationPolicy;
import uz.qoida.platform.courier.domain.DutyState;
...
import uz.qoida.platform.courier.domain.ShiftEnforcement;
import uz.qoida.platform.courier.domain.ShiftStatus;
import uz.qoida.platform.courier.domain.ShiftTransition;
...
import uz.qoida.platform.courier.infrastructure.persistence.JdbcCourierShiftStore.HandoverRow;
...
public class CourierShiftService {
    ...
    public CourierShiftService(JdbcCourierShiftStore shifts, JdbcCourierStore couriers,
            JdbcCourierLedgerStore ledgerStore, JdbcCourierRateCardStore rateCards,
            CourierLedgerService ledger, CourierPolicyResolver policies,
    ...
    public static ApiException notPermitted(ShiftTransition transition, ShiftActor actor) {
        return new ApiException(ErrorCode.INSUFFICIENT_CAPABILITY,
                "%s may not %s a shift; that transition belongs to %s (ADR 0042)"
                        .formatted(actor, transition.name().toLowerCase(java.util.Locale.ROOT),
    ...
    public ShiftRow open(OpenShift command) {
        ...
        EngagementRow engagement = couriers.findLiveEngagement(command.tenantId(), command.courierId())
                .orElseThrow(() -> new ApiException(ErrorCode.UNPROCESSABLE_STATE,
                        "This courier has no live engagement"));
        if (!engagement.status().dispatchable()) {
            throw new ApiException(ErrorCode.UNPROCESSABLE_STATE,
                    ...
                    Map.of("engagementStatus", engagement.status().name(),
                            "warningState", engagement.warningState().name()));
        }
        if (shifts.findLiveShift(command.tenantId(), command.courierId()).isPresent()) {
            throw new ApiException(ErrorCode.RESOURCE_CONFLICT,
                    "This courier already has a live shift");
        }
        ...
        String openPoint = protectPoint(command.tenantId(), shiftId, "protected_open_point",
                command.point());
        PeriodRow period = ledger.currentPeriod(command.tenantId(), command.courierId(),
                command.currency(), LocalDate.ofInstant(clock.instant(), ZoneOffset.UTC));

        shifts.insertShift(new ShiftRow(shiftId, command.tenantId(), command.brandId(),
                command.locationId(), command.courierId(), engagement.id(), ShiftStatus.OPEN,
                DutyState.AVAILABLE, clock.instant(), null, "COURIER", null, null, openPoint,
                null, 0, policy.document().shiftEnforcement(), policy.policyId(),
                policy.policyVersion(), null, period.id(), 1));

        audit.record(fact("courier.shift.opened", command.actor(), command.tenantId(),
                command.brandId(), command.locationId(), shiftId, command.reason(),
                Map.of("openSource", "COURIER",
                        "enforcementMode", policy.document().shiftEnforcement().name(),
                        "enforcementPolicyVersion", policy.policyVersion())));
    ...
    @Transactional
    public void startBreak(UUID tenantId, UUID shiftId, ShiftActor actorKind, ActorRef actor,
            String reason) {
        ...
        if (!shifts.setDutyState(tenantId, shiftId, DutyState.AVAILABLE, DutyState.ON_BREAK,
                clock.instant())) {
            throw new ApiException(ErrorCode.UNPROCESSABLE_STATE,
                    "A break starts from AVAILABLE; this shift is " + shift.dutyState());
        }
        shifts.startBreak(UUID.randomUUID(), tenantId, shiftId, clock.instant());

        audit.record(fact("courier.shift.break-started", actor, tenantId, shift.brandId(),
                shift.locationId(), shiftId, reason, Map.of("dutyState", DutyState.ON_BREAK.name())));
    }

    @Transactional
    public void endBreak(UUID tenantId, UUID shiftId, ShiftActor actorKind, ActorRef actor,
            String reason) {
        ...
        shifts.setDutyState(tenantId, shiftId, DutyState.ON_BREAK, DutyState.AVAILABLE,
                clock.instant());

        audit.record(fact("courier.shift.break-ended", actor, tenantId, shift.brandId(),
                shift.locationId(), shiftId, reason, Map.of("dutyState", DutyState.AVAILABLE.name())));
    }
    ...
    public CloseOutcome close(CloseShift command) {
        ...
        if (command.actorKind() == ShiftActor.MANAGER
            ...
            throw new ApiException(ErrorCode.VALIDATION_FAILED,
                    "A manager close records a reason code");
        }
        ...
        ShiftStatus status = switch (command.actorKind()) {
        ...
        String closeSource = switch (command.actorKind()) {
            case SWEEPER -> "SWEEPER";
            case MANAGER -> "MANAGER";
            case COURIER -> "COURIER";
        };

        String closePoint = protectPoint(command.tenantId(), command.shiftId(),
                "protected_close_point", command.point());
        boolean closed = shifts.close(command.tenantId(), command.shiftId(), status, closeSource,
                command.reasonCode(), closePoint, paidSeconds, breakSeconds, closedAt);
        if (!closed) {
            throw new ApiException(ErrorCode.RESOURCE_CONFLICT,
                    "The shift was closed by somebody else first");
        }
        ...
        audit.record(fact("courier.shift.closed", command.actor(), command.tenantId(),
                shift.brandId(), shift.locationId(), command.shiftId(), command.reason(),
                Map.of("closeSource", closeSource,
                        "status", status.name(),
                        "paidSeconds", paidSeconds,
                        "breakSeconds", breakSeconds,
                        "reasonCode", String.valueOf(command.reasonCode()))));
     ...
     */
    @Transactional
    public void approveHours(UUID tenantId, UUID shiftId, UUID approvalRequestId, ActorRef actor,
            String reason) {
        ...
        creditShiftEarning(shift, shift.paidSeconds() == null ? 0 : shift.paidSeconds(),
                shift.closedAt() == null ? clock.instant() : shift.closedAt());

        audit.record(fact("courier.shift.hours-approved", actor, tenantId, shift.brandId(),
                shift.locationId(), shiftId, reason,
    ...
    public int autoCloseStale(Duration after, String currency) {
        ...
        for (ShiftRow shift : shifts.openBefore(cutoff)) {
            close(new CloseShift(shift.tenantId(), shift.id(), ShiftActor.SWEEPER,
                    ActorRef.systemJob("courier-shift-auto-close"), "AUTO_CLOSED_PAST_SERVICE",
                    "Open past delivery hours plus the configured margin", null, currency));
            closed++;
        }
        return closed;
    }
    ...
    private void creditShiftEarning(ShiftRow shift, long paidSeconds, Instant closedAt) {
        Optional<RateCard> card = rateCards.resolve(shift.tenantId(), shift.brandId(),
                shift.locationId(), courierTypeOf(shift), closedAt);
        if (card.isEmpty()) {
            return;
        }
        CourierAccrual accrual = uz.qoida.platform.courier.domain.AccrualCalculator
                .forShift(card.get(), paidSeconds);
        if (accrual.totalMinor() == 0) {
            return;
        }
        ledger.append(new CourierLedgerService.NewEntry(shift.tenantId(), shift.courierId(),
                shift.locationId(), LedgerEntryType.SHIFT_EARNING, accrual.totalMinor(),
                card.get().currency(), "courier_shift", shift.id(), AdjustmentOrigin.SYSTEM, null,
                closedAt, "shift-earning:" + shift.id(), null, null, "courier-shift-service"));
        ledgerStore.assignShiftToPeriod(shift.tenantId(), shift.id(),
                ledger.currentPeriod(shift.tenantId(), shift.courierId(), card.get().currency(),
                        LocalDate.ofInstant(closedAt, ZoneOffset.UTC)).id());
    }

    private UUID courierTypeOf(ShiftRow shift) {
        return couriers.findCourier(shift.tenantId(), shift.courierId())
                .map(uz.qoida.platform.courier.infrastructure.persistence.JdbcCourierStore.CourierRow::courierTypeId)
                .orElse(null);
    }
    ...
    private UUID openCashHandover(ShiftRow shift, String currency) {
        ...
        shifts.insertHandover(new HandoverRow(handoverId, shift.tenantId(), shift.id(),
                shift.courierId(), shift.locationId(), "PENDING", currency, expected,
    ...
    private AuditFact fact(String action, ActorRef actor, UUID tenantId, UUID brandId,
            UUID locationId, UUID shiftId, String reason, Map<String, Object> changes) {
     ...
     *                 opened for them
     */
    public record OpenShift(UUID tenantId, UUID brandId, UUID locationId, UUID courierId,
            ShiftActor actorKind, ActorRef actor, String reason, String point, String currency) { }

    public record CloseShift(UUID tenantId, UUID shiftId, ShiftActor actorKind, ActorRef actor,
            String reasonCode, String reason, String point, String currency) { }

    public record CloseOutcome(ShiftStatus status, long paidSeconds, long breakSeconds,
            UUID cashHandoverId) { }
