...
public class SecretsConfiguration {
    ...
    @Bean
    @ConditionalOnProperty(name = "qoida.secrets.provider", havingValue = "openbao")
    SecretResolver openBaoSecretResolver(
            @Value("${qoida.secrets.openbao.url}") String url,
            @Value("${qoida.secrets.openbao.token}") String token,
            @Value("${qoida.secrets.openbao.mount:qoida}") String mount,
            Clock clock) {
    ...
    @Bean
    @ConditionalOnProperty(name = "qoida.secrets.provider", havingValue = "environment", matchIfMissing = true)
