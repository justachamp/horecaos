...
public class DataEncryptionKeyProvider {
    ...
    public DataEncryptionKeyProvider(
            ...
            @org.springframework.beans.factory.annotation.Value("${qoida.environment:local}") String environment) {
    ...
    private SecretKey hkdf(KeyIdentity identity) {
        ...
        try {
            return KDF.getInstance("HKDF-SHA256").deriveKey("AES", HKDFParameterSpec.ofExtract()
                    ...
                    // production keys.
                    .addSalt(("qoida:dek:" + environment).getBytes(StandardCharsets.UTF_8))
     ...
     * uz.qoida.platform.iam.api.secrets.SecretValue}, so it is read and let go
    ...
    private record KeyIdentity(UUID tenantId, String dataClass, String generation) {
         ...
         */
        byte[] info() {
            return "qoida:dek|%s|%s|%s".formatted(tenantId, dataClass, generation)
