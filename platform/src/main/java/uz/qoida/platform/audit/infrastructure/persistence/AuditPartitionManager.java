 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.audit.partitions.enabled", havingValue = "true", matchIfMissing = true)
public class AuditPartitionManager {
    ...
    @Scheduled(
            initialDelayString = "${qoida.audit.partitions.initial-delay:PT1M}",
            fixedDelayString = "${qoida.audit.partitions.interval:P1D}")
     ...
     * {@code qoida_application} the CREATE is {@code permission denied for schema
