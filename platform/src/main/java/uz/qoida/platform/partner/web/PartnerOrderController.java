package uz.qoida.platform.partner.web;
...
import uz.qoida.platform.partner.api.PartnerBound;
...
import uz.qoida.platform.web.idempotency.NaturallyIdempotent;

/**
 * The sixth HTTP surface (ADR 0031, ADR 0040).
 *
 * <p>{@code /api/v1/partner/**} joins ADR 0031's five, and it exists so that any
 * aggregator can integrate without a Qoida release. Provider-specific adapters
 ...
 * than a header Qoida does not control the generation of. A retried order that
...
public class PartnerOrderController {
     ...
     * whole evening rather than costing Qoida a few requests.
     */
    ...
    public ResponseEntity<PushResponse> push(
        ...
        if (!outcome.accepted()) {
            // 422 rather than 400: the request was well formed and Qoida
