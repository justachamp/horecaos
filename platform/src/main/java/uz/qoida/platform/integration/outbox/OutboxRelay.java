...
import uz.qoida.platform.integration.retry.RetryBackoff;

@Component
@ConditionalOnProperty(
        name = "qoida.messaging.outbox.enabled",
        havingValue = "true",
        matchIfMissing = true)
public class OutboxRelay {
    ...
    public OutboxRelay(
            ...
            @Value("${qoida.messaging.outbox.batch-size:20}") int batchSize,
            @Value("${qoida.messaging.outbox.lease-duration:5m}") Duration leaseDuration,
            @Value("${qoida.messaging.outbox.publish-timeout:10s}") Duration publishTimeout,
            @Value("${qoida.messaging.outbox.max-attempts:10}") int maxAttempts,
            @Value("${qoida.messaging.outbox.initial-backoff:1s}") Duration initialBackoff,
            @Value("${qoida.messaging.outbox.max-backoff:5m}") Duration maxBackoff) {
        ...
        this.publishedCounter = meterRegistry.counter("qoida.outbox.publications", "outcome", "published");
        this.failedCounter = meterRegistry.counter("qoida.outbox.publications", "outcome", "retry");
        this.deadLetterCounter = meterRegistry.counter("qoida.outbox.publications", "outcome", "dead-letter");
    }

    @Scheduled(fixedDelayString = "${qoida.messaging.outbox.poll-interval:1s}")
