...
@ConditionalOnProperty(
        name = "qoida.messaging.inbox.retry-worker.enabled",
        havingValue = "true",
        matchIfMissing = true)
public class InboxRetryWorker {
    ...
    public InboxRetryWorker(
            ...
            @Value("${qoida.messaging.inbox.retry-batch-size:20}") int batchSize) {
        if (batchSize < 1) {
    ...
    @Scheduled(fixedDelayString = "${qoida.messaging.inbox.retry-interval:5s}")
