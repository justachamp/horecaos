...
import uz.qoida.platform.integration.api.ExternalWorkInboxHandler;
...
import uz.qoida.platform.integration.inbox.EnvelopeValidator.InvalidEnvelopeException;
...
public class InboxExecutor {
    ...
    public InboxExecutor(
            ...
            TransactionTemplate transactions,
            MeterRegistry meters,
            @Value("${qoida.messaging.inbox.max-attempts:10}") int maximumAttempts,
            @Value("${qoida.messaging.inbox.initial-backoff:2s}") Duration initialBackoff,
            @Value("${qoida.messaging.inbox.max-backoff:5m}") Duration maximumBackoff,
            @Value("${qoida.messaging.inbox.blocked-recheck:30s}") Duration blockedRecheckDelay) {
        this.store = store;
    ...
    private void count(String consumerName, String eventType, String outcome) {
        Counter.builder("qoida.inbox.records")
