...
public class JdbcProviderInstallationLookup
    ...
    @Override
    public Optional<String> externalIdFor(UUID bindingId, String entityType, UUID qoidaEntityId) {
        return jdbc.sql("""
                SELECT external_entity_id FROM integration.provider_entity_mappings
                 WHERE binding_id = :bindingId AND entity_type = :entityType
                   AND qoida_entity_id = :qoidaEntityId AND status = 'ACTIVE'
                """)
                .param("bindingId", bindingId)
                .param("entityType", entityType)
                .param("qoidaEntityId", qoidaEntityId)
                .query(String.class)
                .optional();
    }

    @Override
    public Optional<UUID> qoidaIdFor(UUID bindingId, String entityType, String externalId) {
        return jdbc.sql("""
                SELECT qoida_entity_id FROM integration.provider_entity_mappings
                 WHERE binding_id = :bindingId AND entity_type = :entityType
