...
public class KafkaOutboxPublisher implements OutboxPublisher {
    ...
    public void publish(ClaimedOutboxEvent event) throws Exception {
        ...
        addHeader(record, "qoida-event-id", event.eventId().toString());
        addHeader(record, "qoida-event-type", event.eventType());
        addHeader(record, "qoida-event-version", Integer.toString(event.eventVersion()));
        addHeader(record, "qoida-tenant-id", event.tenantId().toString());
        addHeader(record, "qoida-correlation-id", event.correlationId());
