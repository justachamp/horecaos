...
public class CamelNotificationTransport implements NotificationTransport {
    ...
    public DispatchOutcome dispatch(NotificationDispatch dispatch) {
        return translate(send(NotificationRouteBuilder.SEND_ENDPOINT,
                NotificationSendOperation.send(dispatch)));
    }

    @Override
    public DispatchOutcome reconcile(UUID tenantId, UUID brandId, UUID locationId, String channel,
            String providerIdempotencyKey) {
        ProviderOutcome outcome = send(NotificationRouteBuilder.STATUS_ENDPOINT,
                NotificationSendOperation.queryStatus(tenantId, brandId, locationId, channel,
        ...
        if (outcome.status() == ProviderOutcome.Status.REJECTED
            ...
            return DispatchOutcome.retryable("PROVIDER_HAS_NO_RECORD",
                    "The provider has no record of this request", null);
        }
    ...
    private ProviderOutcome send(String endpoint, NotificationSendOperation operation) {
        ...
        ProviderOutcome outcome = result.getMessage()
                .getHeader(NotificationRouteBuilder.OUTCOME_HEADER, ProviderOutcome.class);
    ...
    private static DispatchOutcome translate(ProviderOutcome outcome) {
        DispatchOutcome translated = switch (outcome.status()) {
            case SUCCESS -> DispatchOutcome.accepted(outcome.externalReference(),
                    text(outcome.normalized(), "providerStatus"));
            case REJECTED -> DispatchOutcome.rejected(outcome.errorCode(), outcome.detail());
            case RETRYABLE -> DispatchOutcome.retryable(outcome.errorCode(), outcome.detail(),
        ...
        return bindingId == null
                ? translated
                : translated.from(UUID.fromString(bindingId),
                        text(outcome.normalized(), NotificationGateway.PROVIDER_TYPE_KEY));
    }
