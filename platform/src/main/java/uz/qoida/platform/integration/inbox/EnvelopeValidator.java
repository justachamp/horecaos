...
public class EnvelopeValidator {
    ...
    public EnvelopeValidator(
            ...
            @Value("${qoida.messaging.inbox.max-payload-bytes:262144}") int maximumPayloadBytes) {
        this.objectMapper = objectMapper;
        this.maximumPayloadBytes = maximumPayloadBytes;
    }

    /**
     * @param headers the {@code qoida-*} headers repeated on the Kafka record
    ...
    public ExternalEventEnvelope<JsonNode> validate(
        ...
        requireHeaderMatch(headers, "qoida-event-id", eventId.toString());
        requireHeaderMatch(headers, "qoida-event-type", eventType);
        requireHeaderMatch(headers, "qoida-event-version", String.valueOf(eventVersion));
        requireHeaderMatch(headers, "qoida-tenant-id", tenantId.toString());
        requireHeaderMatch(headers, "qoida-correlation-id", correlationId);
