 ...
 * the argument: everything a customer sees belongs to Qoida, everything that
...
public record FieldAuthorityPolicy(int version, Map<String, FieldAuthority> byField) {
    ...
    public static final FieldAuthorityPolicy INITIAL = new FieldAuthorityPolicy(1, Map.ofEntries(
            // Everything the customer reads is Qoida's. A restaurant renaming a
            ...
            Map.entry("product.name", FieldAuthority.QOIDA),
            Map.entry("product.description", FieldAuthority.QOIDA),
            Map.entry("product.media", FieldAuthority.QOIDA),
            Map.entry("variant.name", FieldAuthority.QOIDA),
            Map.entry("modifier.name", FieldAuthority.QOIDA),
            Map.entry("category.name", FieldAuthority.QOIDA),

            // Price is Qoida's, and this is the entry that survives the worst
            ...
            // pricing incident.
            Map.entry("product.price", FieldAuthority.QOIDA),
            Map.entry("variant.price", FieldAuthority.QOIDA),
            Map.entry("modifier.price", FieldAuthority.QOIDA),

            // What the customer can actually buy is Qoida's, resolved from
            // ADR 0016 offerings and ADR 0017 inventory. The provider's stop list
            // is a strong input to that, not a replacement for it.
            Map.entry("product.available", FieldAuthority.QOIDA),
    ...
    /**
     * @return {@link FieldAuthority#QOIDA} for a field nobody assigned. The safe
    ...
    public FieldAuthority authorityOf(String fieldPath) {
        return byField.getOrDefault(fieldPath, FieldAuthority.QOIDA);
    }
    ...
    public RecommendedAction recommendationFor(String fieldPath) {
        return switch (authorityOf(fieldPath)) {
            ...
            // a Qoida-authoritative field, because the point of the authority is
            // that the provider's value does not get to win.
            case QOIDA -> RecommendedAction.IGNORE;
        };
    }
}
