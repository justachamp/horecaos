...
/**
 * What Qoida currently holds for the entities a binding has mapped (ADR 0012).
 *
 ...
 * authority policy makes every customer-facing name Qoida's, so the provider's
        ...
        jdbc.sql("""
                SELECT m.external_entity_id, p.id, p.version, p.status,
                  ...
                  JOIN catalog.products p
                    ON p.id = m.qoida_entity_id AND p.tenant_id = m.tenant_id
        ...
        jdbc.sql("""
                SELECT m.external_entity_id, v.id, v.version, v.status, v.unit_code,
                  ...
                  JOIN catalog.variants v
                    ON v.id = m.qoida_entity_id AND v.tenant_id = m.tenant_id
                  LEFT JOIN catalog.translations t
