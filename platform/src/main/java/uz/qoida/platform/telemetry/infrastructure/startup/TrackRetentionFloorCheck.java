...
import uz.qoida.platform.telemetry.domain.TrackRetentionFloor.Verdict;
 ...
 * {@code qoida.telemetry.retention.floor-check} is how an operator keeps a real
...
public class TrackRetentionFloorCheck implements ApplicationRunner {
    ...
    public TrackRetentionFloorCheck(JdbcClient jdbc, SettlementCalendarPort calendar,
            ...
            @Value("${qoida.telemetry.retention.floor-check:AUTO}") String mode) {
        this.jdbc = jdbc;
    ...
    public List<Verdict> check() {
        ...
        if (enforcing() && problems.stream().anyMatch(Verdict::refusesStartup)) {
            throw new IllegalStateException("""
                    ...
                    qoida.telemetry.retention.floor-check=REPORT_ONLY for one deployment while
    ...
    private boolean enforcing() {
        ...
        if ("REPORT_ONLY".equalsIgnoreCase(mode)) {
            log.warn("qoida.telemetry.retention.floor-check is REPORT_ONLY. A retention below the "
