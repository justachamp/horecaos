 ...
 * application connects as {@code qoida_application}, which holds no DDL rights on
 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.telemetry.retention.enabled",
...
public class TrackRetentionSweeper {
    ...
    public TrackRetentionSweeper(JdbcClient jdbc, JdbcTelemetryStore store, Clock clock,
            @Value("${qoida.telemetry.retention.report-only:false}") boolean reportOnly,
            @Value("${qoida.telemetry.retention.days:30}") int retentionDays) {
        this.jdbc = jdbc;
        this.store = store;
        this.clock = clock;
    ...
    @Scheduled(
            initialDelayString = "${qoida.telemetry.retention.initial-delay:PT2M}",
            fixedDelayString = "${qoida.telemetry.retention.interval:PT1H}")
    public void sweep() {
        ensurePartitions();
        expireLivePositions();
     ...
     * connected as {@code qoida_application}: the CREATE became {@code permission
