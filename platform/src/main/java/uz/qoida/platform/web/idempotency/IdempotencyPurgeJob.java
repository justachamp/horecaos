 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.api.idempotency.purge.enabled", havingValue = "true", matchIfMissing = true)
public class IdempotencyPurgeJob {
    ...
    @Scheduled(
            initialDelayString = "${qoida.api.idempotency.purge.initial-delay:PT5M}",
            fixedDelayString = "${qoida.api.idempotency.purge.interval:PT1H}")
    public void purge() {
