...
import uz.qoida.platform.ordering.infrastructure.persistence.JdbcOrderStore.DueTimerRow;
 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.ordering.workers.enabled", havingValue = "true",
        matchIfMissing = true)
public class OrderProcessWorker {
    ...
    public OrderProcessWorker(JdbcOrderStore orders, OrderStateService state,
            ...
            @Value("${qoida.ordering.workers.batch-size:50}") int batchSize) {
        this.orders = orders;
        this.state = state;
     ...
     */
    @Scheduled(
            initialDelayString = "${qoida.ordering.workers.timers.initial-delay:PT10S}",
            fixedDelayString = "${qoida.ordering.workers.timers.interval:PT5S}")
    ...
    @Scheduled(
            initialDelayString = "${qoida.ordering.workers.inventory.initial-delay:PT10S}",
            fixedDelayString = "${qoida.ordering.workers.inventory.interval:PT5S}")
