 ...
 * call at all: no create, no query, no refund. The only thing Qoida sends is a
...
public class PaymeProviderAdapter implements PaymentProviderPort {
    ...
    public PaymeProviderAdapter(ProviderInstallationLookup installations,
            ...
            @Value("${qoida.payments.payme.checkout.percent-encode-path-separator:true}")
     ...
     * never happen; stamping a deadline here would be measuring it from Qoida's
    ...
    /**
     * Answers from Qoida's own record, because Payme offers nothing else.
     *
     * <p>{@code CheckTransaction} is a method Payme calls on Qoida, not one Qoida
    ...
    public ProviderOutcome queryOutcome(PaymentAttempt attempt, ProviderBinding binding) {
        ...
        return switch (status) {
            ...
            case UNCERTAIN -> ProviderOutcome.uncertain("PAYME_STATE_UNRESOLVED",
                    "Nothing in Qoida's record settles this attempt");
        };
    }
     ...
     * {@code CancelTransaction}, where Qoida's only lever is to veto it with
    ...
    public ProviderOutcome reverse(PaymentAttempt attempt, ProviderBinding binding, String reason) {
        ...
        return ProviderOutcome.rejected("PAYME_REVERSAL_IS_INBOUND",
                "Payme refunds are initiated in the Payme merchant cabinet and arrive as "
                        + "CancelTransaction; Qoida can only accept or veto them",
