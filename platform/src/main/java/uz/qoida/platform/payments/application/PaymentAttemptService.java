...
import uz.qoida.platform.payments.domain.PaymentAttemptStateMachine;
...
import uz.qoida.platform.payments.domain.ProviderInvoice;
 ...
 * lost, and the sandbox requires the second response to match the first. Qoida's
...
public class PaymentAttemptService {
     ...
     * publishes no response timeout, so this is Qoida's own patience, and its only
     ...
     *
     * <p>Measured from Payme's {@code params.time} and never from Qoida's own
     * creation time. Click imposes no expiry at all, so on Click the equivalent is
     * Qoida's reservation timeout and the provider is never told.
     */
    ...
    private final Map<uz.qoida.platform.payments.domain.PaymentProviderType, PaymentProviderPort> providers;
     ...
     * forever. The cost is that Qoida cancels some transactions Payme might have
    ...
    public int expireStaleReservations(int limit) {
        ...
        for (PaymentAttempt attempt : stale) {
            boolean recorded = recordProviderEvent(attempt, PaymentTransactionType.EXPIRE,
                    ...
                    attempt.providerType() == uz.qoida.platform.payments.domain.PaymentProviderType.PAYME
    ...
    /**
     * Qoida's own transaction id, opaque and non-sequential.
     *
