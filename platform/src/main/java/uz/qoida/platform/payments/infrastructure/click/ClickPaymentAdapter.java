...
import uz.qoida.platform.payments.infrastructure.click.ClickMerchantApi.ClickResponse;

/**
 * Click as {@link PaymentProviderPort} (ADR 0013).
 *
 * <p>Translates between Qoida's states and Click's vocabulary, and nothing else:
...
public class ClickPaymentAdapter implements PaymentProviderPort {
     ...
     * {@code return_url} has told Qoida nothing at all.
     *
     ...
     * eventually releases this order is Qoida's own, and Click is never told about
     * it.
     */
    @Override
     ...
     *
     * <p>Two calls, because the first resolves Qoida's id to Click's and carries no
    ...
    public ProviderOutcome queryOutcome(PaymentAttempt attempt, ProviderBinding binding) {
        ...
        if (paymentId == null) {
            ...
            if (resolved.uncertain() || resolved.status() != uz.qoida.platform.integration.api
