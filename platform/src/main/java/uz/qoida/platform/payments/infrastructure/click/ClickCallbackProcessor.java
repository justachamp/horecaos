...
import uz.qoida.platform.payments.infrastructure.RotationAwareSecrets;
...
import uz.qoida.platform.payments.domain.CallbackKind;
 ...
 * attempt Qoida committed before presenting anything.
 *
...
public class ClickCallbackProcessor {
    ...
    private ClickCallbackDecision decide(ProviderBinding account, String expectedAction,
        ...
        // Click documents no code for this; -8 is Qoida's choice, on the reading
        ...
        // -5. The join key is Qoida's own merchant_trans_id, which is why it is
    ...
    private ClickCallbackDecision complete(ProviderBinding account, PaymentAttempt attempt,
        ...
        if (attempt.status() == PaymentAttemptStatus.INITIATED
            ...
            // Qoida's own Prepare write did not survive, and the reservation is
     ...
     * the hash of the body, the typed identifiers, and the answer Qoida gave — which
    ...
    private void record(ProviderBinding binding, ClickCallbackRequest request,
        ...
        try {
            ...
            // retries a payment Qoida has already credited.
