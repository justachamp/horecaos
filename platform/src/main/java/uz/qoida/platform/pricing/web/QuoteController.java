...
import uz.qoida.platform.pricing.domain.Quote;
...
public class QuoteController {
    ...
    public ResponseEntity<QuoteResponse> quote(
        ...
        var request = new QuoteRequest(tenantId, brandId, body.locationId(),
                body.customerAccountId(), body.channel(),
                body.lines().stream()
                        .map(line -> new QuoteRequest.Line(line.lineId(), line.variantId(),
                                line.quantity(), line.modifierOptionIds()))
                        .toList(),
                idempotencyKey);
        ...
        } catch (QuoteService.NoPublishedMenuException | QuoteService.NoPriceBookException
    ...
    @RequiresCapability(value = Capability.PRICING_READ, scope = ScopeType.BRAND, mutating = true)
    @Operation(summary = "Accept a quote at checkout",
    ...
    public ResponseEntity<AcceptanceResponse> accept(
            @PathVariable UUID tenantId, @PathVariable UUID brandId, @PathVariable UUID quoteId,
        ...
        return switch (acceptance.outcome()) {
            case ACCEPTED -> ResponseEntity.ok(new AcceptanceResponse(
                    acceptance.outcome().name(), acceptance.total().minor(),
            ...
            case PRICE_CHANGED -> throw new ApiException(ErrorCode.PRICE_CHANGED,
                    "The price changed; request a new quote before accepting");
            case EXPIRED -> throw new ApiException(ErrorCode.RESOURCE_CONFLICT,
                    "This quote has expired or was already accepted");
        };
    }
    ...
    public record QuoteResponse(
            UUID quoteId, String currency, String contextHash,
            long subtotalMinor, long taxMinor, long totalMinor,
            Instant expiresAt, List<LineResponse> lines, List<AdjustmentResponse> adjustments) {

        static QuoteResponse of(Quote quote) {
            return new QuoteResponse(
                    quote.quoteId(), quote.currency(), quote.contextHash(),
                    quote.subtotal().minor(), quote.tax().minor(), quote.total().minor(),
                    quote.expiresAt(),
                    quote.lines().stream()
                            .map(line -> new LineResponse(line.lineId(), line.variantId(),
                                    line.quantity(), line.descriptionSnapshot(),
                                    line.unitAmount().minor(), line.finalAmount().minor(),
                                    line.taxAmount().minor()))
                            .toList(),
                    quote.adjustments().stream()
                            .map(a -> new AdjustmentResponse(a.sequence(), a.lineId(),
                                    a.type().name(), a.descriptionCode(), a.amount().minor()))
                            .toList());
        }
    }

    public record LineResponse(String lineId, UUID variantId, int quantity, String description,
            long unitAmountMinor, long finalAmountMinor, long taxAmountMinor) { }

    /** Every step that made up the total, so "why is this 47,000 som" has an answer. */
    public record AdjustmentResponse(int sequence, String lineId, String type,
