 ...
 */
@Component
@ConditionalOnProperty(name = "qoida.customers.verification.sweep.enabled",
...
public class VerificationChallengeSweeper {
    ...
    public VerificationChallengeSweeper(
            ...
            Clock clock,
            @Value("${qoida.customers.verification.retention:P30D}") Duration retention,
            @Value("${qoida.customers.verification.sweep.batch-size:1000}") int batchSize) {
    ...
    @Scheduled(
            initialDelayString = "${qoida.customers.verification.sweep.initial-delay:PT2M}",
            fixedDelayString = "${qoida.customers.verification.sweep.interval:PT15M}")
    public void sweep() {
