...
public class DeliverySourcingRunner {
    ...
    public DeliverySourcingRunner(DeliverySourcingService sourcing, SourcingJournal journal,
            ...
            @Value("${qoida.fulfillment.sourcing.initial-backoff:5s}") Duration initialBackoff,
            @Value("${qoida.fulfillment.sourcing.max-backoff:2m}") Duration maxBackoff,
            @Value("${qoida.fulfillment.sourcing.max-attempts:12}") int maxAttempts) {
