...
@ConditionalOnProperty(
        name = "qoida.fulfillment.sourcing.enabled",
        havingValue = "true",
        matchIfMissing = true)
public class DeliverySourcingScheduler {
    ...
    public DeliverySourcingScheduler(JdbcSourcingJobStore jobs, DeliverySourcingRunner runner,
            ...
            @Value("${qoida.fulfillment.sourcing.batch-size:5}") int batchSize,
            @Value("${qoida.fulfillment.sourcing.lease-duration:3m}") Duration lease,
            @Value("${qoida.fulfillment.sourcing.tick-timeout:20s}") Duration tickTimeout,
        ...
        this.sourcedCounter = meterRegistry.counter("qoida.delivery.sourcing.ticks",
                "outcome", "decided");
        this.failedCounter = meterRegistry.counter("qoida.delivery.sourcing.ticks",
                "outcome", "failed");
    }

    @Scheduled(fixedDelayString = "${qoida.fulfillment.sourcing.poll-interval:1s}")
