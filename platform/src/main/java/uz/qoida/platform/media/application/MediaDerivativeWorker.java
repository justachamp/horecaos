...
import uz.qoida.platform.media.infrastructure.persistence.JdbcDerivativeJobStore.ClaimedJob;
...
@ConditionalOnProperty(
        name = "qoida.media.derivatives.enabled",
        havingValue = "true",
        matchIfMissing = true)
public class MediaDerivativeWorker {
    ...
    public MediaDerivativeWorker(
            ...
            @Value("${qoida.media.derivatives.batch-size:4}") int batchSize,
            @Value("${qoida.media.derivatives.lease-duration:5m}") Duration lease,
            @Value("${qoida.media.derivatives.initial-backoff:30s}") Duration initialBackoff,
            @Value("${qoida.media.derivatives.max-backoff:15m}") Duration maximumBackoff,
            @Value("${qoida.media.derivatives.max-attempts:6}") int maximumAttempts,
            @Value("${spring.application.name:qoida-platform}") String applicationName) {
        ...
        this.rendered = meters.counter("qoida.media.derivative.jobs", "outcome", "rendered");
        this.abandoned = meters.counter("qoida.media.derivative.jobs", "outcome", "abandoned");
        this.retried = meters.counter("qoida.media.derivative.jobs", "outcome", "retried");
    }

    @Scheduled(fixedDelayString = "${qoida.media.derivatives.poll-interval:2s}")
