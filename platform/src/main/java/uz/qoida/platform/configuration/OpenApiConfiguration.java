...
public class OpenApiConfiguration {
    ...
    @Bean
    OpenAPI qoidaOpenApi(@Value("${qoida.api.version}") String apiVersion) {
        SecurityScheme bearerScheme = new SecurityScheme()
                .type(SecurityScheme.Type.HTTP)
                .scheme("bearer")
                .bearerFormat("JWT")
                .description("Keycloak access token issued for the qoida-api audience");

        return new OpenAPI()
                .info(new Info()
                        .title("Qoida Platform API")
                        .version(apiVersion)
                        .description("Tenant-aware APIs for the Qoida SaaS commerce and delivery platform."))
