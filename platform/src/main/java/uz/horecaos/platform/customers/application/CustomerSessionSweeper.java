...
public class CustomerSessionSweeper {
    ...
    public CustomerSessionSweeper(
            ...
            Clock clock,
            @Value("${horecaos.customers.session.retention:P7D}") Duration grace,
            @Value("${horecaos.customers.session.sweep.batch-size:1000}") int batchSize) {
    ...
    @Scheduled(
            initialDelayString = "${horecaos.customers.session.sweep.initial-delay:PT3M}",
            fixedDelayString = "${horecaos.customers.session.sweep.interval:PT1H}")
    public void sweep() {
