...
public class BotCallbackAuthorizer {
    ...
    public Outcome decide(String token, long telegramUserId) {
        ...
        if (resolved.isEmpty()) {
            return new Outcome(Result.TOKEN_EXPIRED, null, null);
        }
        ...
        if (!entitlements.featureEnabled(action.tenantId(), EntitlementKeys.TELEGRAM_BOT_INTERACTIVE_ENABLED)) {
            return new Outcome(Result.NOT_ENTITLED, null, null);
        }

        Optional<String> principal = staffLinks.principalFor(action.tenantId(), telegramUserId);
        if (principal.isEmpty()) {
            return new Outcome(Result.NOT_LINKED, null, null);
        }
        ...
        try {
            ...
            // ever touched or not.
            return new Outcome(Result.UNAUTHORIZED, subject, null);
        }

        OrderDecisionPort.Decision decision = orderDecisions.decide(
                action.tenantId(),
                action.orderId(),
                new OrderDecisionPort.DecisionCommand(token, action.action(), subject, clock.instant(), token));

        Result result = decision.applied() ? Result.APPLIED : Result.ALREADY_SETTLED;
        return new Outcome(result, subject, decision);
    }

    /** What happened, for {@link TelegramUpdateHandler} to render. */
    public record Outcome(Result result, @Nullable String actorSubject, OrderDecisionPort.@Nullable Decision decision) {
