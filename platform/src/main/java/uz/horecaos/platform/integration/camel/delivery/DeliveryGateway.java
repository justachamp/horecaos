...
public class DeliveryGateway {
    ...
    public DeliveryGateway(
            List<DeliveryPartner> partners, ProviderInstallationLookup installations, SecretResolver secrets) {
    ...
    public ProviderOutcome createShipment(BindingRef binding, DeliveryRequest request, String idempotencyKey) {
        ...
        return invoke(binding, required, idempotencyKey, (partner, call) -> partner.createShipment(request, call));
    }
    ...
    public ProviderOutcome cancelShipment(
            BindingRef binding, String externalReference, String reason, String idempotencyKey) {
        return invoke(
                binding,
    ...
    private ProviderOutcome invoke(
            ...
            BiFunction<DeliveryPartner, ProviderCall, ProviderOutcome> operation) {
        ...
        ImportSuppression.refuse(
                ExternalEffect.COURIER_BOOKING, "%s on %s".formatted(capability, binding.providerType()));

        Optional<DeliveryPartner> resolved = partner(binding);
        if (resolved.isEmpty()) {
            return ProviderOutcome.rejected(
                    "NO_ADAPTER", "No delivery adapter is registered for " + binding.providerType());
        }
        ...
        if (!partner.supports(capability)) {
            ...
            return ProviderOutcome.rejected(
                    "CAPABILITY_UNSUPPORTED", "%s does not support %s".formatted(binding.providerType(), capability));
        }
        ...
        if (snapshot.isEmpty()) {
            return ProviderOutcome.rejected(
                    "INSTALLATION_MISSING", "Installation " + binding.installationId() + " is not available");
        }
        ...
        if (isAuthenticationFailure(outcome)) {
            ...
            outcome = operation.apply(
                    partner, new ProviderCall(installation.baseUrl(), fresh.reveal(), idempotencyKey, DEFAULT_TIMEOUT));
        }
        return outcome;
    }
