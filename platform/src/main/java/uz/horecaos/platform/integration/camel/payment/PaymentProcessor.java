...
public class PaymentProcessor {
    ...
    public PaymentProcessor(PaymentGateway gateway, PaymentCircuitBreakers breakers, MeterRegistry meters) {
    ...
    public void recordOutcome(Exchange exchange) {
        MerchantApiCall call = call(exchange);
        ProviderOutcome outcome = exchange.getIn().getHeader(PaymentRouteBuilder.OUTCOME_HEADER, ProviderOutcome.class);
    ...
    public void deadLetter(Exchange exchange) {
        ...
        log.error(
                "Payment call {} on {} dead-lettered as {}",
                call.operation(),
                call.providerType(),
                outcome.errorCode());
        PaymentRouteBuilder.clearContext();
