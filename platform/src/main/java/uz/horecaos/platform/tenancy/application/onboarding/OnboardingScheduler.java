 ...
 * {@code infra/observability/horecaos-probe.sh}, which is where every ADR 0023
 ...
 */
@Component
@ConditionalOnProperty(name = "horecaos.onboarding.scheduler.enabled", havingValue = "true", matchIfMissing = true)
public class OnboardingScheduler {
    ...
    public OnboardingScheduler(
            ...
            Clock clock,
            @Value("${horecaos.onboarding.scheduler.batch-size:5}") int batchSize) {
        this.jdbc = jdbc;
        this.onboarding = onboarding;
        this.clock = clock;
        this.batchSize = batchSize;

        meters.gauge("horecaos.onboarding.runs.waiting", this, OnboardingScheduler::runsWaiting);
        meters.gauge("horecaos.onboarding.runs.failed", this, OnboardingScheduler::runsFailed);
        meters.gauge("horecaos.onboarding.runs.stalled.age.seconds", this, OnboardingScheduler::stalledAgeSeconds);
    }

    @Scheduled(
            initialDelayString = "${horecaos.onboarding.scheduler.initial-delay:PT10S}",
            fixedDelayString = "${horecaos.onboarding.scheduler.interval:PT5S}")
    public void drive() {
