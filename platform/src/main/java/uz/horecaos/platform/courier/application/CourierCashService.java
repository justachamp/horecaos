...
public class CourierCashService {
    ...
    public CourierCashService(
            JdbcCourierShiftStore shifts, CourierLedgerService ledger, AuditRecorder audit, Clock clock) {
        this.shifts = shifts;
        this.ledger = ledger;
        this.audit = audit;
        this.clock = clock;
    ...
    @Transactional
    public HandoverRow declare(UUID tenantId, UUID handoverId, UUID courierId, long declaredMinor, ActorRef actor) {
        ...
        if (!handover.courierId().equals(courierId)) {
            ...
            throw new ApiException(ErrorCode.RESOURCE_NOT_FOUND, "No such cash handover of yours");
        }
        if (!shifts.declare(tenantId, handoverId, declaredMinor, clock.instant())) {
            throw new ApiException(ErrorCode.UNPROCESSABLE_STATE, "This handover is " + handover.status());
        }
    ...
    public HandoverRow confirm(
        ...
        if (handover.declaredMinor() == null) {
            throw new ApiException(ErrorCode.UNPROCESSABLE_STATE, "Nothing has been declared on this handover yet");
        }
        ...
        if (!shifts.confirm(
                tenantId, handoverId, confirmedMinor, variance, status, reasonCode, actor.subject(), clock.instant())) {
            throw new ApiException(ErrorCode.RESOURCE_CONFLICT, "This handover was confirmed by somebody else first");
        }
        ...
        ledger.append(new CourierLedgerService.NewEntry(
                ...
                handover.courierId(),
                handover.locationId(),
                LedgerEntryType.CASH_HANDED_OVER,
                confirmedMinor,
                handover.currency(),
    ...
    private HandoverRow handover(UUID tenantId, UUID handoverId) {
        return shifts.findHandover(tenantId, handoverId)
                .orElseThrow(
                        () -> new ApiException(ErrorCode.RESOURCE_NOT_FOUND, "No such cash handover: " + handoverId));
    }
}
