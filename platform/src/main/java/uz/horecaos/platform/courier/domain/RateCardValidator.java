...
public final class RateCardValidator {
    ...
    public static void validateForActivation(RateCard card) {
        ...
        for (int index = 1; index < bands.size(); index++) {
            ...
            if (previous.bandToMeters() == null) {
                throw new InvalidRateCardException("An unbounded band at %d metres is followed by another band"
        ...
        if (bands.getLast().bandToMeters() != null) {
            throw new InvalidRateCardException("The last distance band must be unbounded; this card stops at %d metres"
    ...
    private static void requireSomethingToPay(RateCard card) {
        boolean paysSomething = card.components().stream().anyMatch(component -> component.amountMinor() > 0);
        if (!paysSomething) {
            throw new InvalidRateCardException("A rate card that pays nothing cannot be activated; archive it instead");
        }
    }
}
