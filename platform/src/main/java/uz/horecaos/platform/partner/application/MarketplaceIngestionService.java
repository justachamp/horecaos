...
import uz.horecaos.platform.partner.domain.ExternalReferenceType;
 ...
 * stronger key: HorecaOS does not control the partner's retry client, while the
...
public class MarketplaceIngestionService {
     ...
     *
     * <p>The aggregator took the customer's money before HorecaOS heard of the order,
     * so there is exactly one tender and it is neither cash nor a HorecaOS provider.
    ...
    public Outcome receive(PartnerPrincipal principal, PartnerOrderPush push) {
        ...
        try {
            ...
            // HorecaOS had already refused and have the restatement win silently.
     ...
     * {@link uz.horecaos.platform.payments.settlement.OrderSettlementService#refund}
