package uz.horecaos.platform.ordering.infrastructure.customer;
...
public class JdbcCustomerAddressBook implements CustomerAddressBook {
    ...
    public JdbcCustomerAddressBook(JdbcClient jdbc, FieldProtection protection, ObjectMapper objectMapper) {
        this.jdbc = jdbc;
    ...
    private SavedDestination reveal(UUID tenantId, AddressRow row, String purpose) {
        String document = protection.reveal(
                ...
                new RecordRef(ADDRESS_TABLE, FIELDS_COLUMN, row.id()),
                purpose);
        ...
        String instructions = row.encryptedInstructions() == null
                ? null
                : protection.reveal(
                        tenantId,
                        ProtectedValue.deserialize(row.encryptedInstructions()),
                        new RecordRef(ADDRESS_TABLE, INSTRUCTIONS_COLUMN, row.id()),
                        purpose);
