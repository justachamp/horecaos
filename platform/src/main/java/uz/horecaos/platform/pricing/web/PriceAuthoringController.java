...
public class PriceAuthoringController {
    ...
    @Operation(
            summary = "Create a draft price book",
    ...
    public ResponseEntity<PriceBookResponse> create(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
            @Valid @RequestBody CreatePriceBookRequest request) {

        return respond(authoring.create(
                tenantId,
                brandId,
    ...
    public ResponseEntity<PriceBookResponse> read(
            @PathVariable UUID tenantId, @PathVariable UUID brandId, @PathVariable UUID priceBookId) {
    ...
    @RequiresCapability(value = Capability.PRICING_AUTHOR, scope = ScopeType.BRAND, mutating = true)
    @Operation(
            summary = "Apply a price book to the whole brand",
            description =
                    "The fallback every location and channel resolves to when nothing " + "more specific applies.")
    public ResponseEntity<PriceBookResponse> assignToBrand(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
        ...
        return respond(guarded(() ->
                authoring.assign(tenantId, brandId, priceBookId, AssignmentScope.BRAND, null, request.toAssignment())));
    }

    @PutMapping("/price-books/{priceBookId}/assignments/locations/{locationId}")
    @RequiresCapability(value = Capability.PRICING_AUTHOR, scope = ScopeType.LOCATION, mutating = true)
    @Operation(
            summary = "Apply a price book to one branch",
    ...
    public ResponseEntity<PriceBookResponse> assignToLocation(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
            @PathVariable UUID priceBookId,
        ...
        return respond(guarded(() -> authoring.assign(
                tenantId, brandId, priceBookId, AssignmentScope.LOCATION, locationId, request.toAssignment())));
    }

    @PutMapping("/price-books/{priceBookId}/assignments/channels/{channelId}")
    @RequiresCapability(value = Capability.PRICING_AUTHOR, scope = ScopeType.BRAND, mutating = true)
    @Operation(
            summary = "Apply a price book to one sales channel",
            description = "Outranks a branch book: an aggregator's agreed prices must not "
                    + "depend on which branch fulfils the order.")
    public ResponseEntity<PriceBookResponse> assignToChannel(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
            @PathVariable UUID priceBookId,
        ...
        return respond(guarded(() -> authoring.assign(
                tenantId, brandId, priceBookId, AssignmentScope.CHANNEL, channelId, request.toAssignment())));
    }
    ...
    @RequiresCapability(value = Capability.PRICING_AUTHOR, scope = ScopeType.BRAND, mutating = true)
    @Operation(
            summary = "Set what a modifier option costs",
    ...
    public ResponseEntity<PriceBookResponse> setModifierOptionPrice(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
            @PathVariable UUID priceBookId,
            @PathVariable UUID modifierOptionId,
            @Valid @RequestBody PriceRequest request) {
    ...
    @Operation(
            summary = "Put a price book in front of customers",
    ...
    public ResponseEntity<PriceBookResponse> activate(
            @PathVariable UUID tenantId,
            @PathVariable UUID brandId,
            @PathVariable UUID priceBookId,
     ...
     */
    private static PriceAuthoringService.PriceBook guarded(Supplier<PriceAuthoringService.PriceBook> call) {
        try {
            return call.get();
    ...
    public record AssignmentRequest(@PositiveOrZero Integer priority, Instant validFrom, Instant validUntil) {

        PriceAuthoringService.Assignment toAssignment() {
            return new PriceAuthoringService.Assignment(priority == null ? 0 : priority, validFrom, validUntil);
        }
    }
