 ...
 *
 * <p>Narrow by design, the same shape {@link SampleMenuPricingPort} already
 * takes: a caller outside {@code pricing} cannot reach {@code
 * PriceAuthoringService} at all under Spring Modulith. Unlike the sample
 * menu's port, this one writes and reads a single variant per call rather than
 * a batch, because {@code CatalogImportRowService} processes one CSV row per
 * transaction — the same reason {@code CustomerCsvImportRowService} is its own
 * bean, see that class's own doc — and a row that fails to price must not roll
 * back a price a previous row in the same file already committed.
 *
 * <p>Every price this port writes lands in one dedicated price book per brand,
 * {@value #CATALOG_IMPORT_PRICE_BOOK_NAME}, created on first use and kept
 * active. This is a deliberate scope limitation, not an oversight: a full
 * import of "the" price a variant should carry would mean choosing among the
 * brand's price books, resolving priority against whatever the operator has
 * already authored by hand, and re-running publication — decisions ADR 0018
 * leaves to a person, not a file. What this port gives a re-imported file is
 * the property that matters for a dry run to be honest: {@link
...
public interface CatalogImportPricingPort {
     ...
     *     reason a retry cannot fix (the same permanent-refusal shape {@link
     ...
     */
    Optional<Long> currentPrice(UUID tenantId, UUID brandId, UUID variantId);
