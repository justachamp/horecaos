...
@Tag(name = "Storefront payments", description = "Opening a payment attempt and getting the checkout surface for it")
public class StorefrontPaymentController {
    ...
    public StorefrontPaymentController(
            PaymentCheckoutService checkout, OrderDirectory orders, CurrentCustomer currentCustomer) {
    ...
    @Idempotent
    @Operation(
            summary = "Open a payment attempt and get its checkout surface",
    ...
    public record PaymentSessionRequest(
            ...
            @Size(max = 512) @Pattern(regexp = "^https://.*", message = "a return URL must be https")
            String returnUrl,
