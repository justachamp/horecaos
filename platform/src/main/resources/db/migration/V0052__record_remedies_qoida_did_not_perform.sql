...
-- provider's own cabinet; Qoida records it so the order, the settlement and the
-- analytics stay whole.
--
-- The cost of that decision is that this table asserts something Qoida did not
...
CREATE TABLE payments.order_remedies (
    ...
    -- Qoida moved plus what somebody says they moved elsewhere.
...
COMMENT ON COLUMN payments.order_remedies.attested_money_minor IS
    'Money a person asserts left a merchant account Qoida cannot see. Unverifiable by construction, and reported apart from platform_settled_minor everywhere.';
...
GRANT SELECT, INSERT, UPDATE ON payments.order_remedies TO qoida_application;
GRANT SELECT, INSERT, UPDATE ON payments.remedy_entitlements TO qoida_application;
-- Append-only, said as a grant rather than as a convention.
GRANT SELECT, INSERT ON payments.entitlement_redemptions TO qoida_application;
