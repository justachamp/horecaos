...
class DifferenceEngineTests {
    ...
    @Test
    @DisplayName("a HorecaOS-owned field never produces an applicable action")
    void aCuratedNameIsRecordedAndNotApplied() {
        var target = new TargetCatalog(Map.of(
                EntityType.PRODUCT,
                Map.of("41", new TargetCatalog.Entity(PRODUCT, 3, Map.of("product.name", "Lagman, hand-pulled")))));
    ...
    void oneMissedReadIsInconclusive() {
        ...
        var result = engine.compare(snapshot(List.of(), List.of(), false), target, AbsenceHistory.empty());
    ...
    void twoAgreeingRunsMakeARemovalActionable() {
        var target = new TargetCatalog(
                Map.of(EntityType.PRODUCT, Map.of("41", new TargetCatalog.Entity(PRODUCT, 1, Map.of()))));
        var history = new AbsenceHistory(Map.of(EntityType.PRODUCT, Map.of("41", new AbsenceHistory.Streak(1, false))));
    ...
    void aStableWalkMakesOneAbsenceEvidence() {
        ...
        var result = engine.compare(snapshot(List.of(), List.of(), true), target, AbsenceHistory.empty());

        assertThat(result.differences())
                .singleElement()
                .satisfies(
                        difference -> assertThat(difference.category()).isEqualTo(DifferenceCategory.REMOVAL_SIGNAL));
    }
    ...
    void aDuplicateIsAConflict() {
        var result = engine.compare(
                snapshot(
                        List.of(
                                ...
                                product("41", "Lagman", SourceKind.DISH, 33000L)),
                        List.of(),
                        false),
                TargetCatalog.empty(),
                AbsenceHistory.empty());
        ...
        assertThat(result.differences())
                ...
                .noneSatisfy(
                        difference -> assertThat(difference.externalEntityId()).isEqualTo("41"));
    }
    ...
    void anUnrepresentableModifierIsAConflict() {
        var snapshot = new CatalogSnapshot(
                ...
                1,
                List.of(),
                List.of(product("7", "Cola", SourceKind.GOODS, 9000L)),
                List.of(),
    ...
    void inventoryKindsAreNotMenuCandidates() {
        var result = engine.compare(
                snapshot(List.of(product("3", "Test_Tomato", SourceKind.INGREDIENT, 0L)), List.of(), false),
                TargetCatalog.empty(),
                AbsenceHistory.empty());
    ...
    void aMissingParentIsAConflict() {
        var snapshot = new CatalogSnapshot(
                ...
                List.of(),
                List.of(),
                List.of(new CatalogSnapshot.Variant("410", "41", "Large", 36000L, "UZS", true, "1", Map.of())),
                List.of(),
                List.of(),
                List.of());
        ...
        assertThat(result.conflicts())
                .singleElement()
                .satisfies(conflict -> assertThat(conflict.kind()).isEqualTo(SyncConflict.Kind.MISSING_PARENT));
    }

    private static CatalogSnapshot snapshot(
            List<CatalogSnapshot.Product> products, List<CatalogSnapshot.Variant> variants, boolean walkStable) {
