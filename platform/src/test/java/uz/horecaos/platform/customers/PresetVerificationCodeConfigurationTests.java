...
/**
 * Whether {@link PresetVerificationCodeSource}'s four locks actually boot the way
...
class PresetVerificationCodeConfigurationTests {
    ...
    private static final String ENVIRONMENT_PROPERTY = PresetVerificationCodeSource.ENVIRONMENT_PROPERTY;
    ...
    @Test
    @DisplayName("preprod, configured, but horecaos.environment says the real production: refused at startup")
    void preprodWithRealProductionEnvironmentRefusesToStart() {
        runner.withPropertyValues(
                ...
                .run(context -> {
                    assertThat(context)
                            .as("the Spring profile alone cannot prove this host is not the real "
                                    + "production environment; horecaos.environment is the signal that can, "
                                    + "and the bean's own constructor is where that is checked")
                            .hasFailed();
                    assertThat(context.getStartupFailure())
                            .hasRootCauseInstanceOf(IllegalStateException.class)
                            .rootCause()
                            .hasMessageContaining(PHONE_PROPERTY)
                            .hasMessageContaining(ENVIRONMENT_PROPERTY);
                });
    }
}
