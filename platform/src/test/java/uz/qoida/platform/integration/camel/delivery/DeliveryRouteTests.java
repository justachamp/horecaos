...
import uz.qoida.platform.integration.api.delivery.DeliveryCapability;
import uz.qoida.platform.integration.api.delivery.DeliveryPartner;
...
class DeliveryRouteTests {
    ...
    void uncertainCreateNeverRepeatsTheSideEffect() throws Exception {
        ...
        ProviderOutcome outcome = run(partner, operation(
                DeliveryCapability.CREATE_ON_DEMAND_SHIPMENT, "ext-1"));
    ...
    void uncertainCreateWithoutAReferenceIsEscalated() throws Exception {
        ...
        ProviderOutcome outcome = run(partner, operation(
                DeliveryCapability.CREATE_ON_DEMAND_SHIPMENT, null));
    ...
    void businessRejectionIsNotRetried() throws Exception {
        ScriptedPartner partner = new ScriptedPartner()
                .onCreate(ProviderOutcome.rejected("OUT_OF_ZONE", "address outside coverage"));
    ...
    void unsupportedCapabilityShortCircuits() throws Exception {
        ...
        ProviderOutcome outcome = run(partner, operation(
                DeliveryCapability.RESCHEDULE_SHIPMENT, "ext-1"));
    ...
    void retryableIsSurfacedNotLooped() throws Exception {
        ScriptedPartner partner = new ScriptedPartner()
                .onQuery(ProviderOutcome.retryable("PROVIDER_ERROR_503", "unavailable",
    ...
    void crossTenantBindingIsRefused() {
        BindingRef otherTenant = new BindingRef(UUID.randomUUID(), INSTALLATION, UUID.randomUUID(),
        ...
        assertThat(org.assertj.core.api.Assertions.catchThrowable(() -> new DeliveryOperation(
                UUID.randomUUID(), TENANT, otherTenant, DeliveryCapability.QUERY_SHIPMENT,
                null, "ext-1", null, "corr-1")))
    ...
    void providerFaultsOpenTheCircuit() throws Exception {
        ScriptedPartner partner = new ScriptedPartner()
                .onQuery(ProviderOutcome.retryable("PROVIDER_ERROR_503", "unavailable", null));

        List<ProviderOutcome> outcomes = runMany(partner, 20,
                () -> operation(DeliveryCapability.QUERY_SHIPMENT, "ext-1"));
    ...
    void circuitsAreIndependentPerProvider() throws Exception {
        ...
        List<ProviderOutcome> outcomes = runMany(List.of(failing, healthy), 40,
                alternating(), meters);
    ...
    void circuitOpenMetricIsNotEmittedForOrdinaryFaults() throws Exception {
        ...
        assertThat(meters.find("qoida.delivery.route").tag("event", "circuit_open").counter())
                .isNull();
    }
    ...
    void businessRejectionsDoNotOpenTheCircuit() throws Exception {
        ScriptedPartner partner = new ScriptedPartner()
                .onQuery(ProviderOutcome.rejected("OUT_OF_ZONE", "address outside coverage"));
    ...
    void anUnsettledReconciliationBecomesADurableCommand() throws Exception {
        ...
        assertThat(outcome.status()).isEqualTo(ProviderOutcome.Status.UNCERTAIN);
        assertThat(reconciliations.requested())
                .singleElement()
                .satisfies(command -> {
    ...
    private List<ProviderOutcome> runMany(ScriptedPartner partner, int times,
            java.util.function.Supplier<DeliveryOperation> operations) throws Exception {
        return runMany(List.of(partner), times, operations, new SimpleMeterRegistry());
    }

    private List<ProviderOutcome> runMany(List<DeliveryPartner> partners, int times,
            java.util.function.Supplier<DeliveryOperation> operations, SimpleMeterRegistry meters)
            throws Exception {
        ...
        try (ProducerTemplate template = camel.createProducerTemplate()) {
            for (int attempt = 0; attempt < times; attempt++) {
                outcomes.add(template.request(DeliveryRouteBuilder.OPERATION_ENDPOINT,
                                exchange -> exchange.getIn().setBody(operations.get()))
                        .getIn().getHeader(DeliveryRouteBuilder.OUTCOME_HEADER, ProviderOutcome.class));
            }
        }
        return outcomes;
    }
    ...
    private ProviderOutcome run(ScriptedPartner partner, DeliveryOperation operation) throws Exception {
        DeliveryGateway gateway = new DeliveryGateway(
                List.of(partner), lookup(), fixedResolver());
        SimpleMeterRegistry meters = new SimpleMeterRegistry();
        DeliveryProcessor processor = new DeliveryProcessor(gateway,
                new DeliveryCircuitBreakers(meters, Clock.systemUTC()), meters,
                reconciliations = new RecordingReconciliationOutbox());
        ...
        try (ProducerTemplate template = camel.createProducerTemplate()) {
            return template.request(DeliveryRouteBuilder.OPERATION_ENDPOINT,
    ...
    private static DeliveryOperation operationFor(String providerType,
            DeliveryCapability capability, String reference) {
        BindingRef binding = new BindingRef(UUID.randomUUID(), INSTALLATION, TENANT,
        ...
        DeliveryPartner.DeliveryRequest request = new DeliveryPartner.DeliveryRequest(
                ...
                null, true, 1000L, "UZS", Map.of());
        return new DeliveryOperation(UUID.randomUUID(), TENANT, binding, capability,
                request, reference, "test", "corr-1");
    }
