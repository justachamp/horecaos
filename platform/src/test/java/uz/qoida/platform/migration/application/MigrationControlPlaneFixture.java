...
import uz.qoida.platform.audit.infrastructure.persistence.JdbcApprovalRequestOwnership;
...
import uz.qoida.platform.iam.infrastructure.authorization.JdbcAuthorizationService;
import uz.qoida.platform.iam.infrastructure.authorization.RoleRegistrySynchronizer;
...
import uz.qoida.platform.migration.infrastructure.persistence.JdbcCutoverDecisionStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcEntityMappingStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcMigrationProgramStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcMigrationRunStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcMigrationScopeStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcQuarantineStore;
import uz.qoida.platform.migration.infrastructure.persistence.JdbcReconciliationStore;
 ...
 *
 * <p>The test classes sit in {@code uz.qoida.platform.migration.application}
...
abstract class MigrationControlPlaneFixture {
    ...
    void setUp() {
        ...
        programId = programs.create(new MigrationProgramService.CreateProgramCommand(
                "Delever cutover", "delever-prod", "qoida-prod", 3, "seeding the suite")).id();
    }
    ...
    protected UUID openScope(MigrationCapability capability, UUID brandId, UUID locationId) {
        return programs.openScope(programId, new OpenScopeCommand(TENANT, brandId, locationId,
                capability, "DELEVER", "QOIDA_ORDERING", "opening for the suite")).id();
    }
