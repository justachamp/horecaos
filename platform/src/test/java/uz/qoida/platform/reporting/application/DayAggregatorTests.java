...
import uz.qoida.platform.reporting.application.ReportingFacts.BranchDayAggregate;
import uz.qoida.platform.reporting.application.ReportingFacts.OrderFact;
import uz.qoida.platform.reporting.application.ReportingFacts.RefundFact;
...
class DayAggregatorTests {
    ...
    void twoLegalEntitiesProduceTwoRowsAndNeverOne() {
        ...
        List<BranchDayAggregate> rows = DayAggregator.branchDay(DAY,
                List.of(completed("ord-1", ENTITY_A, 100_000, 0),
                        completed("ord-2", ENTITY_B, 60_000, 0)),
                List.of(), 1, 1);

        assertThat(rows).hasSize(2);
        assertThat(rows).extracting(row -> row.key().legalEntityId())
                .containsExactlyInAnyOrder(ENTITY_A, ENTITY_B);
        assertThat(rows).extracting(BranchDayAggregate::grossSom)
                .containsExactlyInAnyOrder(100_000L, 60_000L);
    }
    ...
    void aRefundLandsOnItsOwnDayAndNotTheOrdersDay() {
        ...
        // exactly as it was.
        RefundFact refund = new RefundFact(TENANT, DAY, UUID.randomUUID(), UUID.randomUUID(),
                DAY.minusDays(3), LOCATION, ENTITY_A, "TELEGRAM", "DELIVERY", 25_000,
                Instant.parse("2026-08-21T14:00:00Z"), 1, 1);

        List<BranchDayAggregate> rows = DayAggregator.branchDay(DAY,
                List.of(completed("ord-1", ENTITY_A, 100_000, 0)), List.of(refund), 1, 1);
    ...
    void aRefundForABranchWithNoOrdersTodayStillGetsARow() {
        ...
        RefundFact refund = new RefundFact(TENANT, DAY, UUID.randomUUID(), UUID.randomUUID(),
                DAY.minusDays(9), LOCATION, ENTITY_A, "TELEGRAM", "DELIVERY", 40_000,
                Instant.parse("2026-08-21T14:00:00Z"), 1, 1);

        List<BranchDayAggregate> rows =
                DayAggregator.branchDay(DAY, List.of(), List.of(refund), 1, 1);
    ...
    void aDayWithNothingClosedHasNoAverageRatherThanAZeroSecondOne() {
        OrderFact stillOpen = new OrderFact(TENANT, UUID.randomUUID(), DAY, 1,
                Instant.parse("2026-08-21T14:00:00Z"), null, UUID.randomUUID(), LOCATION,
                ENTITY_A, "TELEGRAM", "DELIVERY", "PREPARING", null, null, null,
                50_000, 0, 0, 0, 50_000, 1, 1, 30, null, null, null, null, null, 1, 1);

        List<BranchDayAggregate> rows =
                DayAggregator.branchDay(DAY, List.of(stillOpen), List.of(), 1, 1);
    ...
    void theSlaSharesSumToTheWhole() {
        ...
        assertThat(buckets.stream().mapToInt(bucket -> bucket.shareBasisPoints()).sum())
                .isEqualTo(10_000);
        assertThat(buckets).extracting(bucket -> bucket.bucketCode())
    ...
    void anOrderStillOpenIsNotBucketed() {
        ...
        OrderFact open = new OrderFact(TENANT, UUID.randomUUID(), DAY, 1,
                Instant.parse("2026-08-21T14:00:00Z"), null, UUID.randomUUID(), LOCATION,
                ENTITY_A, "TELEGRAM", "DELIVERY", "PREPARING", null, null, null,
                50_000, 0, 0, 0, 50_000, 1, 1, null, null, null, null, null, null, 1, 1);
    ...
    void latenessCountsOnlyOrdersThatCarriedAPromise() {
        ...
        OrderFact promisedAndLate = new OrderFact(TENANT, UUID.randomUUID(), DAY, 1, placed,
                placed.plusSeconds(3600), UUID.randomUUID(), LOCATION, ENTITY_A, "TELEGRAM",
                "DELIVERY", "COMPLETED", null, null, null, 50_000, 0, 0, 0, 50_000, 1, 1,
                60, 1800, 3600, placed.plusSeconds(2400), null, 1200, 1, 1);
        OrderFact neverPromised = new OrderFact(TENANT, UUID.randomUUID(), DAY, 1, placed,
                placed.plusSeconds(7200), UUID.randomUUID(), LOCATION, ENTITY_A, "TELEGRAM",
                "DELIVERY", "COMPLETED", null, null, null, 50_000, 0, 0, 0, 50_000, 1, 1,
                60, 1800, 7200, null, null, null, 1, 1);

        var rows = DayAggregator.branchDay(DAY, List.of(promisedAndLate, neverPromised),
                List.of(), 1, 1);

        assertThat(rows.getFirst().promisedCount())
                .as("an unpromised order cannot be late, and counting it as on time is a "
                        + "different lie")
                .isEqualTo(1);
    ...
    private static OrderFact completed(String seed, UUID entityId, long gross, long discount) {
        Instant placed = Instant.parse("2026-08-21T14:00:00Z");
        return new OrderFact(TENANT, UUID.nameUUIDFromBytes(seed.getBytes()), DAY, 1, placed,
                placed.plusSeconds(2400), UUID.randomUUID(), LOCATION, entityId, "TELEGRAM",
                "DELIVERY", "COMPLETED", null, null, null,
                gross, discount, 0, 0, gross - discount, 2, 3, 60, 1500, 2400, null, null, null,
                1, 1);
    }
    ...
    private static OrderFact closedAfter(String seed, int seconds) {
        Instant placed = Instant.parse("2026-08-21T14:00:00Z");
        return new OrderFact(TENANT, UUID.nameUUIDFromBytes(seed.getBytes()), DAY, 1, placed,
                placed.plusSeconds(seconds), UUID.randomUUID(), LOCATION, ENTITY_A, "TELEGRAM",
                "DELIVERY", "COMPLETED", null, null, null,
                50_000, 0, 0, 0, 50_000, 1, 1, 60, 1200, seconds, null, null, null, 1, 1);
    }
}
