...
import uz.qoida.platform.pricing.application.PromotionEvaluator;
import uz.qoida.platform.pricing.application.PromotionEvaluator.Basket;
import uz.qoida.platform.pricing.application.PromotionEvaluator.BasketLine;
import uz.qoida.platform.pricing.application.PromotionEvaluator.Outcome;
import uz.qoida.platform.pricing.application.PromotionEvaluator.PromotionContext;
import uz.qoida.platform.pricing.domain.Promotion;
import uz.qoida.platform.pricing.domain.Promotion.Action;
import uz.qoida.platform.pricing.domain.Promotion.Condition;
import uz.qoida.platform.pricing.domain.Promotion.Operands;
...
class PromotionEvaluatorTests {
    ...
    void itemPercentageAppliesToMatchingLinesOnly() {
        Outcome outcome = evaluator.evaluate(
                List.of(itemPercentOffProduct("OSH10", "SEASONAL", OSH, 1_000)),
                basketWithBoth(), context(), NOW);
    ...
    void bestOneWinsWithinAStackingGroup() {
        ...
        Outcome outcome = evaluator.evaluate(List.of(weaker, stronger), basketWithBoth(),
                context(), NOW);

        assertThat(outcome.applied())
                .as("one promotion applies, not both")
                .hasSize(1);
    ...
    void differentStackingGroupsCombine() {
        ...
        Outcome outcome = evaluator.evaluate(List.of(seasonal, loyalty), basketWithBoth(),
                context(), NOW);
    ...
    void exclusiveSuppressesEverythingElse() {
        Promotion exclusive = new Promotion(UUID.randomUUID(), TENANT, BRAND, "LAUNCH",
                Promotion.Scope.ITEM, "LAUNCH", true, 0, false, null, "UZS", NOW.minusSeconds(60),
                null, 1, List.of(productIs(OSH)), List.of(percentAction(1_000)));
        Promotion betterButNotExclusive = itemPercentOffProduct("OSH50", "SEASONAL", OSH, 5_000);

        Outcome outcome = evaluator.evaluate(List.of(exclusive, betterButNotExclusive),
                basketWithBoth(), context(), NOW);
    ...
    void orderDiscountRunsOnTheReducedSubtotal() {
        ...
        Outcome outcome = evaluator.evaluate(List.of(item, order), basketWithBoth(),
                context(), NOW);
    ...
    void thresholdSeesTheReducedSubtotal() {
        Promotion item = itemPercentOffProduct("OSH50", "SEASONAL", OSH, 5_000);
        Promotion needsFifty = new Promotion(UUID.randomUUID(), TENANT, BRAND, "OVER50",
                Promotion.Scope.ORDER, "ORDERWIDE", false, 0, false, null, "UZS",
                NOW.minusSeconds(60), null, 1,
                List.of(new Condition(1, Condition.Type.SUBTOTAL_AT_LEAST,
                        new Operands(Map.of("amountMinor", 50_000L)))),
                List.of(new Action(1, Action.Type.ORDER_FIXED_DISCOUNT,
                        new Operands(Map.of("amountMinor", 5_000L)))));

        Outcome outcome = evaluator.evaluate(List.of(item, needsFifty), basketWithBoth(),
                context(), NOW);
    ...
    void couponPromotionsNeedTheirCode() {
        Promotion coupon = new Promotion(UUID.randomUUID(), TENANT, BRAND, "SUMMER20",
                Promotion.Scope.ORDER, "COUPON", false, 0, true, null, "UZS", NOW.minusSeconds(60),
                null, 1, List.of(), List.of(new Action(1, Action.Type.ORDER_PERCENTAGE_DISCOUNT,
                        new Operands(Map.of("basisPoints", 2_000L)))));
        ...
        PromotionContext presented = new PromotionContext("STOREFRONT", LOCATION, "DELIVERY",
                false, Set.of(), Set.of(coupon.promotionId()), 5, 12 * 60);
    ...
    void aPromotionOutsideItsWindowDoesNotApply() {
        Promotion expired = new Promotion(UUID.randomUUID(), TENANT, BRAND, "OLD",
                Promotion.Scope.ITEM, "SEASONAL", false, 0, false, null, "UZS",
                NOW.minusSeconds(7200), NOW.minusSeconds(3600), 1,
                List.of(productIs(OSH)), List.of(percentAction(5_000)));

        assertThat(evaluator.evaluate(List.of(expired), basketWithBoth(), context(), NOW).applied())
                .isEmpty();
    }
    ...
    void theCapHoldsAndThePartsReconcile() {
        Promotion capped = new Promotion(UUID.randomUUID(), TENANT, BRAND, "CAPPED",
                Promotion.Scope.ITEM, "SEASONAL", false, 0, false, 3_000L, "UZS",
                NOW.minusSeconds(60), null, 1, List.of(), List.of(percentAction(5_000)));
        ...
        long summed = outcome.lineDiscountsMinor().values().stream().mapToLong(Long::longValue).sum();
        assertThat(summed)
    ...
    void selectionComparesWhatTheCustomerActuallyReceives() {
        Promotion cappedGiant = new Promotion(UUID.randomUUID(), TENANT, BRAND, "BIGBUTCAPPED",
                Promotion.Scope.ITEM, "SEASONAL", false, 0, false, 5_000L, "UZS",
                NOW.minusSeconds(60), null, 1, List.of(productIs(OSH)), List.of(percentAction(9_000)));
        Promotion flat = new Promotion(UUID.randomUUID(), TENANT, BRAND, "FLAT10K",
                Promotion.Scope.ITEM, "SEASONAL", false, 0, false, null, "UZS",
                NOW.minusSeconds(60), null, 1, List.of(productIs(OSH)),
                List.of(new Action(1, Action.Type.ITEM_FIXED_DISCOUNT,
                        new Operands(Map.of("amountMinor", 10_000L)))));

        Outcome outcome = evaluator.evaluate(List.of(cappedGiant, flat), basketWithBoth(),
                context(), NOW);
    ...
    void aFixedDiscountCannotGoNegative() {
        Promotion huge = new Promotion(UUID.randomUUID(), TENANT, BRAND, "HUGE",
                Promotion.Scope.ITEM, "SEASONAL", false, 0, false, null, "UZS",
                NOW.minusSeconds(60), null, 1, List.of(productIs(SOMSA)),
                List.of(new Action(1, Action.Type.ITEM_FIXED_DISCOUNT,
                        new Operands(Map.of("amountMinor", 999_999L)))));
    ...
    void combinedOrderDiscountsAreCappedAtTheBasket() {
        ...
        Outcome outcome = evaluator.evaluate(List.of(first, second), basketWithBoth(),
                context(), NOW);
    ...
    void aWrappingWindowIncludesTheSmallHours() {
        Promotion lateNight = new Promotion(UUID.randomUUID(), TENANT, BRAND, "NIGHT",
                Promotion.Scope.ORDER, "NIGHT", false, 0, false, null, "UZS", NOW.minusSeconds(60),
                null, 1,
                List.of(new Condition(1, Condition.Type.TIME_OF_DAY, new Operands(
                        Map.of("fromMinuteOfDay", 22 * 60, "toMinuteOfDay", 2 * 60)))),
                List.of(new Action(1, Action.Type.ORDER_FIXED_DISCOUNT,
                        new Operands(Map.of("amountMinor", 5_000L)))));

        PromotionContext oneInTheMorning = new PromotionContext("STOREFRONT", LOCATION, "DELIVERY",
                false, Set.of(), Set.of(), 5, 60);
        assertThat(evaluator.evaluate(List.of(lateNight), basketWithBoth(), oneInTheMorning, NOW)
                .orderDiscountMinor())
                .as("01:00 falls inside a 22:00-02:00 window")
                .isEqualTo(5_000L);

        PromotionContext midday = new PromotionContext("STOREFRONT", LOCATION, "DELIVERY",
                false, Set.of(), Set.of(), 5, 12 * 60);
        assertThat(evaluator.evaluate(List.of(lateNight), basketWithBoth(), midday, NOW)
                .orderDiscountMinor())
                .as("midday does not")
                .isZero();
    }
    ...
    void tiesAreBrokenDeterministically() {
        ...
        String forwards = evaluator.evaluate(List.of(left, right), basketWithBoth(), context(), NOW)
                .applied().get(0).code();
        String backwards = evaluator.evaluate(List.of(right, left), basketWithBoth(), context(), NOW)
                .applied().get(0).code();

        assertThat(forwards)
                .as("row order must never decide a discount")
                .isEqualTo(backwards);
    }
    ...
    void anUnmatchedProductDoesNotApply() {
        ...
        assertThat(evaluator.evaluate(List.of(other), basketWithBoth(), context(), NOW).applied())
                .isEmpty();
    }
    ...
    void freeDeliveryIsCappedAtTheFee() {
        Promotion free = new Promotion(UUID.randomUUID(), TENANT, BRAND, "FREEDEL",
                Promotion.Scope.DELIVERY, "DELIVERY", false, 0, false, null, "UZS",
                NOW.minusSeconds(60), null, 1, List.of(),
    ...
    private PromotionContext context() {
        return new PromotionContext("STOREFRONT", LOCATION, "DELIVERY", false, Set.of(), Set.of(),
                5, 12 * 60);
    }

    private Promotion itemPercentOffProduct(String code, String group, UUID productId, int basisPoints) {
        return new Promotion(UUID.randomUUID(), TENANT, BRAND, code, Promotion.Scope.ITEM, group,
                false, 0, false, null, "UZS", NOW.minusSeconds(60), null, 1,
                List.of(productIs(productId)), List.of(percentAction(basisPoints)));
    }

    private Promotion orderPercent(String code, String group, int basisPoints) {
        return new Promotion(UUID.randomUUID(), TENANT, BRAND, code, Promotion.Scope.ORDER, group,
                false, 0, false, null, "UZS", NOW.minusSeconds(60), null, 1, List.of(),
                List.of(new Action(1, Action.Type.ORDER_PERCENTAGE_DISCOUNT,
                        new Operands(Map.of("basisPoints", (long) basisPoints)))));
    }

    private Condition productIs(UUID productId) {
        return new Condition(1, Condition.Type.PRODUCT,
                new Operands(Map.of("productIds", List.of(productId.toString()))));
    }

    private Action percentAction(int basisPoints) {
        return new Action(1, Action.Type.ITEM_PERCENTAGE_DISCOUNT,
                new Operands(Map.of("basisPoints", (long) basisPoints)));
    }
}
