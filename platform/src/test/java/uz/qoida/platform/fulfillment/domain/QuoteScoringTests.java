...
import uz.qoida.platform.fulfillment.domain.sourcing.DeliveryQuote;
...
import uz.qoida.platform.fulfillment.domain.sourcing.QuoteScoring;
import uz.qoida.platform.fulfillment.domain.sourcing.QuoteScoring.ScoredPartner;
...
class QuoteScoringTests {
    ...
    private static final PartnerOption NOOR = new PartnerOption(
            UUID.fromString("aaaaaaaa-0000-0000-0000-000000000001"), "noor-delivery", false, true);
    private static final PartnerOption YANDEX = new PartnerOption(
            UUID.fromString("aaaaaaaa-0000-0000-0000-000000000002"), "yandex-delivery", true, true);

    @Test
    @DisplayName("the cheaper partner is asked first however the bindings were configured")
    void theCheaperPartnerIsAskedFirst() {
        List<ScoredPartner> scored = QuoteScoring.rank(List.of(NOOR, YANDEX),
                List.of(quote(NOOR, 28_000L, 480), quote(YANDEX, 19_000L, 600)), plan(), NOW);
    ...
    @Test
    @DisplayName("a courier who cannot reach the branch before the window shuts is excluded, "
    ...
    void aPartnerThatCannotMakeTheWindowIsExcluded() {
        ...
        List<ScoredPartner> scored = QuoteScoring.rank(List.of(NOOR, YANDEX),
                List.of(quote(NOOR, 5_000L, 2_400), quote(YANDEX, 19_000L, 600)), plan(), NOW);
    ...
    @Test
    @DisplayName("a quote past the TTL Qoida imposed is not an answer any more")
    void anExpiredQuoteIsNotUsed() {
        DeliveryQuote stale = new DeliveryQuote(UUID.randomUUID(), NOOR.bindingId(),
                "noor-delivery", UUID.randomUUID(), 5_000L, "UZS", 300, 1_200, 3_400, 900,
                NOW.minusSeconds(1), false, null, NOW.minusSeconds(200));

        List<ScoredPartner> scored = QuoteScoring.rank(List.of(NOOR, YANDEX),
                List.of(stale, quote(YANDEX, 19_000L, 600)), plan(), NOW);
    ...
    void anUnquotedPartnerIsStillBookable() {
        List<ScoredPartner> scored = QuoteScoring.rank(List.of(NOOR, YANDEX),
                List.of(quote(YANDEX, 19_000L, 600)), plan(), NOW);
    ...
    void tieBreakingIsDeterministic() {
        ...
        List<PartnerOption> oneWay = QuoteScoring.order(
                QuoteScoring.rank(List.of(NOOR, YANDEX), List.of(noor, yandex), plan(), NOW));
        List<PartnerOption> theOther = QuoteScoring.order(
                QuoteScoring.rank(List.of(NOOR, YANDEX), List.of(yandex, noor), plan(), NOW));
    ...
    void aRefusalIsCarriedIntoTheSelection() {
        DeliveryQuote refused = new DeliveryQuote(UUID.randomUUID(), NOOR.bindingId(),
                "noor-delivery", UUID.randomUUID(), null, null, null, null, null, null,
                NOW.plusSeconds(120), false, "CancelledOutOfZone", NOW);

        List<ScoredPartner> scored = QuoteScoring.rank(List.of(NOOR, YANDEX),
                List.of(refused, quote(YANDEX, 19_000L, 600)), plan(), NOW);
    ...
    private static DeliveryQuote quote(PartnerOption partner, long priceMinor, int pickupEta) {
        return new DeliveryQuote(UUID.randomUUID(), partner.bindingId(), partner.providerType(),
                UUID.randomUUID(), priceMinor, "UZS", pickupEta, 1_500, 3_400, 900,
                NOW.plusSeconds(120), false, null, NOW);
    }

    private static PickupPlan plan() {
        return PickupPlan.forOrder(NOW, Duration.ofMinutes(15), ZoneId.of("Asia/Tashkent"),
