...
import uz.qoida.platform.ordering.infrastructure.persistence.JdbcOrderProcessStore.ProcessRow;
...
class OrderInventoryProcessTests {
    ...
    void setUp() {
        ...
        process = new OrderInventoryProcess(processes, inventory, JsonMapper.builder().build(),
    ...
    void oneUnrunnableRowIsNotEverybodysProblem() {
        when(processes.claim(eq(OrderInventoryProcess.PROCESS_NAME), any(), anyInt()))
                .thenReturn(List.of(
                        row(STUCK_ORDER, """
                                {"action":"RETURN","quoteId":"%s"}""".formatted(QUOTE), 0, 3),
                        row(GOOD_ORDER, """
        ...
        assertThat(settled)
                ...
                .isEqualTo(2);
        verify(processes).settle(eq(GOOD_ORDER), anyString(), eq(1), eq("COMPLETED"), anyString(),
                eq(null), eq(null), eq(NOW));
        verify(processes).settle(eq(STUCK_ORDER), anyString(), eq(3), eq("FAILED_RETRYABLE"),
                anyString(), any(Instant.class), anyString(), eq(NOW));
    }
    ...
    void aQuarantinedRowKeepsItsCheckpoint() {
        ...
        verify(processes).settle(eq(STUCK_ORDER), anyString(), anyInt(), eq("FAILED_RETRYABLE"),
                written.capture(), any(Instant.class), anyString(), eq(NOW));
    ...
    void theLadderReachesAnOperator() {
        ...
        // belongs.
        verify(processes).settle(eq(STUCK_ORDER), anyString(), eq(9), eq("MANUAL_ACTION_REQUIRED"),
                anyString(), eq(null), anyString(), eq(NOW));
    }
    ...
    private static ProcessRow row(UUID orderId, String checkpoint, int attemptCount, int version) {
        return new ProcessRow(orderId, OrderInventoryProcess.PROCESS_NAME, TENANT, "WAITING",
                checkpoint, attemptCount, version);
    }
}
