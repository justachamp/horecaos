...
export const REFRESH_TOKEN_KEY = 'horecaos.operations.refreshToken';

/**
 * This console's own tokens (ADR 0035, ADR 0062).
 *
 ...
 *
 * The refresh token now lives in `sessionStorage`, which it did not before.
 * The previous, memory-only version of this class was ADR 0035's letter but
 * not its point — its own doc named the actual cost plainly: "a page refresh
 * drops the session and the operator sees the sign-in page again." That was
 * never a deliberate security boundary; it was the leftover shape of
 * `SplitSecurityStorage`, a class built to keep a PKCE verifier and a CSRF
 * `state` alive across the redirect to Keycloak, which ADR 0062 then deleted
 * the need for entirely. In practice it meant F5, a shared order link, or a
 * till tablet coming back from sleep after the tab's own memory state was
 * gone all dumped an operator mid-shift back to `/login`, losing whatever
 * screen they had open — the order board, an order they were three taps into
 * building.
 *
 ...
 * closing and reopening — so a stolen till tablet, rebooted days later, could
 * still resurrect a staff session from disk. `sessionStorage` cannot: close
 * the tab, or the browser, and this console asks for a password again
 * exactly as it always did. Nothing here changes how long a *live* session
 * can run unattended — that is Keycloak's refresh-token lifetime, set
 * server-side — only whether a page load has to throw the session away for
 * no reason.
 */
