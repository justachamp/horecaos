...
export class OrderDetailComponent implements OnInit {
  ...
  /**
   * The resolved branch's address, joined the same way
   * `CartConfirmationComponent.pickupLocationAddress` joins it: only the
   * parts `StorefrontLocationProfileController`'s `LocationProfile` actually
   * sent (`addressLine`/`district`/`city` are all optional), so a branch
   * missing one is never shown with a trailing ", ".
   */
  pickupBranchAddress(): string {
    const branch = this.pickupBranch();
    return [branch?.addressLine, branch?.district, branch?.city]
