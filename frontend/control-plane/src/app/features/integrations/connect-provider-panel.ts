 ...
 * doc comment names.
 *
 * A right-side drawer rather than {@link RotateSecretDialog}'s centred modal:
 * a connect form has more fields than a rotation does, and a drawer is the
 * natural shape for that, the same distinction the design-system prototype's
 * `Drawer` and `Modal` primitives draw.
 */
@Component({
  ...
  template: `
    <div class="backdrop" (click)="cancel.emit()">
      <div class="drawer" role="dialog" aria-modal="true" [attr.aria-label]="i18n.t('integrations.connect.title')" (click)="$event.stopPropagation()">
        <div class="header">
          <h2 class="q-subhead">{{ i18n.t('integrations.connect.title') }}</h2>
          <button type="button" class="q-body close" (click)="cancel.emit()" aria-label="Close">✕</button>
        </div>

        <div class="body">
          <label class="q-caption field-label" for="connect-provider">{{ i18n.t('integrations.connect.provider') }}</label>
          <select
            id="connect-provider"
            class="q-body field"
            [value]="providerType()"
          ...
          <label class="q-caption field-label" for="connect-display-name">{{ i18n.t('integrations.connect.displayName') }}</label>
          <input
          ...
          <label class="q-caption field-label" for="connect-environment">{{ i18n.t('integrations.connect.environmentCode') }}</label>
          <input
            ...
            [disabled]="submitting()"
          />
          <p class="q-caption hint">{{ i18n.t('integrations.connect.environmentCode.hint') }}</p>

          @for (field of selectedDeclaration()?.fields ?? []; track field.key) {
            <label class="q-caption field-label" [for]="'connect-field-' + field.key">{{ label(field.key) }}</label>
            <input
        ...
        <div class="actions">
          <button type="button" class="q-body secondary" (click)="cancel.emit()" [disabled]="submitting()">
            {{ i18n.t('integrations.connect.cancel') }}
          </button>
          <button type="button" class="q-body primary" (click)="submit()" [disabled]="!canSubmit()">
            {{ submitting() ? i18n.t('integrations.connect.submitting') : i18n.t('integrations.connect.submit') }}
          </button>
        </div>
      </div>
    </div>
