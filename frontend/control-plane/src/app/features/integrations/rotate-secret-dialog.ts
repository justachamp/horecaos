 ...
 * same {@code {value, reason}} shape, and this is the one form for it.
 *
 * A centred modal rather than a side drawer, following {@code
 * order-reason-dialog} in the sibling `operations` app: a short form fits it,
 * where the longer connect flow below wants the drawer instead.
 *
 * Mount/unmount rather than a service-driven overlay stack — the parent
 * renders this with `@if (rotating(); as target) { <app-rotate-secret-dialog ... /> }`
 * and owns whether it exists at all, so there is nothing here to leak if the
 * parent forgets to close it.
 */
@Component({
  ...
  template: `
    <div class="backdrop" (click)="cancel.emit()">
      <div class="dialog" role="dialog" aria-modal="true" [attr.aria-label]="i18n.t('integrations.rotate.title')" (click)="$event.stopPropagation()">
        <h2 class="q-subhead title">{{ i18n.t('integrations.rotate.title') }}</h2>
        <p class="q-body-sm lead">{{ i18n.t('integrations.rotate.lead') }}</p>

        @if (unverifiable()) {
          <p class="q-body-sm notice">{{ i18n.t('integrations.rotate.unverifiedNotice') }}</p>
        }

        <label class="q-caption field-label" for="rotate-value">{{ i18n.t('integrations.rotate.value') }}</label>
        <input
          id="rotate-value"
          class="q-body field"
          type="password"
        ...
        <label class="q-caption field-label" for="rotate-reason">{{ i18n.t('integrations.rotate.reason') }}</label>
        <input
          id="rotate-reason"
          class="q-body field"
          type="text"
        ...
        <div class="actions">
          <button type="button" class="q-body secondary" (click)="cancel.emit()" [disabled]="submitting()">
            {{ i18n.t('integrations.rotate.cancel') }}
          </button>
          <button type="button" class="q-body primary" (click)="submit()" [disabled]="!canSubmit()">
            {{ submitting() ? i18n.t('integrations.rotate.submitting') : i18n.t('integrations.rotate.submit') }}
          </button>
        </div>
      </div>
    </div>
